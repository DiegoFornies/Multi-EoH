...
def heuristic(input_data):
    """Combines SPT job order with earliest machine availability."""
    ...
    for job in job_order:
        ...
        for op_idx, (machines, times) in enumerate(jobs_data[job]):
            ...
            for m_idx, machine in enumerate(machines):
                potential_start_time = max(machine_available_time[machine], current_job_start_time)
                if potential_start_time < best_time:
                    best_machine = machine
                    best_time = potential_start_time
            ...
            current_job_start_time = end_time
