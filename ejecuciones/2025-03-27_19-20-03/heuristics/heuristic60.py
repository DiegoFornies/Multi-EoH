...
def heuristic(input_data):
    """Schedules jobs to minimize makespan and balance machine load using SPT."""
    ...
    while priority_list:
        ...
        # Shortest Processing Time (SPT) heuristic: Select machine that results shortest operation time
        # within machine's avaiability
        for m, t in zip(machines, times):
            start_time = max(machine_time[m], job_completion_time[job])
            if start_time + t < min_start_time:
                min_start_time = start_time + t # track earliest completion time
                best_machine = m
                best_time = t
