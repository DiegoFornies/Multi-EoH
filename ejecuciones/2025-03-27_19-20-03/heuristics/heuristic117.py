...
def heuristic(input_data):
    """Schedules jobs by minimizing idle time and balancing machine load."""
    ...
    for job in range(1, n_jobs + 1):
        ...
        for op_idx, (machines, times) in enumerate(jobs_data[job]):
            ...
            # Find the machine that minimizes idle time, then machine load.
            best_machine = None
            min_idle = float('inf')
            ...
            for m_idx, machine in enumerate(machines):
                ...
                idle_time = max(0, machine_available_time[machine] - current_time) # idle time if scheduled here
