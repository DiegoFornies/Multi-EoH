...
def heuristic(input_data):
    """Hybrid heuristic: SPT priority, machine load, and job completion time."""
    ...
    while priority_queue:
        ...
        min_machine_load = float('inf') #added machine load

        for m_idx, m in enumerate(machines):
            processing_time = times[m_idx]
            start_time = max(machine_time[m], job_completion_time[job])
            end_time = start_time + processing_time
            machine_load = machine_time[m] #Consider current machine load

            if start_time < min_start_time or (start_time == min_start_time and machine_load < min_machine_load) :
