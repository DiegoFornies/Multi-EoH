...
def heuristic(input_data):
    ...
    # Initial priority based on SPT (Shortest Processing Time) and job due date
    for job in jobs:
        ops = jobs[job]
        machines, times = ops[0]
        min_time = min(times)
        priority = min_time  # SPT-based priority
    ...
    while priority_list:
        ...
        # Choose machine based on earliest finish time (EFT)
        for m, t in zip(machines, times):
            start_time = max(machine_time[m], job_completion_time[job])
            finish_time = start_time + t

            if finish_time < min_start_time:
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = job_completion_time[job] + min_time #Based on SPT for subsequent ops
            priority_list.append((priority, job))
            priority_list.sort()
