...
def heuristic(input_data):
    """Schedules jobs to minimize makespan and balance machine load. Prioritizes operations based on slack time and load balancing."""
    ...
    # Initial priority: Completion Time + Processing Time - Balancing Machine Load
    for job in jobs:
        ops = jobs[job]
        machines, times = ops[0]
        min_time = min(times)
        priority = min_time + (job/n_jobs) # Initial priority
    ...
    while priority_list:
        ...
        # Prioritize machines with lower loads if multiple options exist.
        for m, t in zip(machines, times):
            start_time = max(machine_time[m], job_completion_time[job])
            load_penalty = machine_load[m] / (sum(machine_load.values()) + 1e-9) #Avoid zero division
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = job_completion_time[job] + min_time # Use slack
            priority_list.append((priority, job))
            priority_list.sort()
