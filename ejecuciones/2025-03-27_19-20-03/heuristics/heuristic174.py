...
def heuristic(input_data):
    ...
    # Initial priority: Earliest possible start time + job/n_jobs for tiebreaker
    for job in jobs:
        ops = jobs[job]
        machines, times = ops[0]
        min_time = min(times)
        priority = min_time + (job/n_jobs) # Earliest start time first
    ...
    while priority_list:
        ...
        # Prioritize machines with lower load, break ties with earliest start.
        for m, t in zip(machines, times):
            start_time = max(machine_time[m], job_completion_time[job])
            load_penalty = machine_load[m] / (sum(machine_load.values()) + 1e-9)
            adjusted_start_time = start_time + load_penalty * t  # Scale penalty by processing time
        ...
        if best_machine is None:
            raise ValueError("No machine available for operation")
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = job_completion_time[job] + min_next_time + (job/n_jobs) #Slack time based
            priority_list.append((priority, job))
            priority_list.sort()
