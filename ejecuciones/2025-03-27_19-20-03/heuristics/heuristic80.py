...
def heuristic(input_data):
    """Schedules jobs minimizing makespan, using SPT and load balancing."""
    ...
    for job in jobs:
        ...
        min_time = min(times)
        priority = min_time # SPT-like initial priority
    ...
    while priority_list:
        ...
        for m, t in zip(machines, times):
            ...
            # Bias towards less loaded machines
            load_factor = machine_load[m] / sum(machine_load.values()) if sum(machine_load.values()) > 0 else 0
            adjusted_start_time = start_time + load_factor * 100 # Add penalty
        ...
        if best_machine is None:
            raise ValueError("No machine found")
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = job_completion_time[job] + min_time # SPT-like
            priority_list.append((priority, job))
            priority_list.sort()
