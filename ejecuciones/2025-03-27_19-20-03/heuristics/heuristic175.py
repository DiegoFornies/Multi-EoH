...
def heuristic(input_data):
    """FJSSP heuristic: SPT-based priority with machine load balance."""
    ...
    # Initial priority: (processing time / number of machines) + job_id
    for job in jobs:
        ops = jobs[job]
    ...
    while priority_queue:
        ...
        for m_idx, m in enumerate(machines):
            ...
            # Add a load-balancing term to the start time to avoid overloading
            load_penalty = machine_load[m] / (sum(machine_load.values()) + 1e-9)  # Avoid division by zero
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = job_completion_time[job] + (min_time / len(next_machines)) + (job/n_jobs) #Combine slack with SPT
            priority_queue.append((priority, job))
            priority_queue.sort()
