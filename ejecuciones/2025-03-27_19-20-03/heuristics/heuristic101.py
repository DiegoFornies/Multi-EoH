...
def heuristic(input_data):
    """Schedules jobs considering due dates (job number), SPT, and machine load."""
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            for m, t in zip(next_machines, next_times):
                if t < min_time:
                    min_time = t
                    min_machine = m
            priority = job_completion_time[job] + min_time + job #EDD + SPT
            priority_list.append((priority, job))
            priority_list.sort()
