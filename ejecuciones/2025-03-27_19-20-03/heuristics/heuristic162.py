...
def heuristic(input_data):
    ...
    priority_list = []
    for job in jobs:
        priority = job_remaining_times[job]
    ...
    while priority_list:
        ...
        if job_ops[job] < len(jobs[job]):
            priority = job_completion_time[job] + job_remaining_times[job] - sum(schedule[job][k]['Processing Time'] for k in range(len(schedule[job])))
            priority_list.append((priority, job))
            priority_list.sort()
