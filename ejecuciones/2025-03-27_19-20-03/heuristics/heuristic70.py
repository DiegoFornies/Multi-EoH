...
def heuristic(input_data):
    ...
    operation_count = {j: 0 for j in range(1, n_jobs + 1)}

    job_ops = {job: 0 for job in jobs}
    priority_list = []

    # Initialize priority based on shortest processing time (SPT)
    for job in jobs:
        ops = jobs[job]
    ...
    while priority_list:
        ...
        for m, t in zip(machines, times):
            ...
            # Prioritize machine with lowest load
            load_factor = machine_load[m] / (sum(machine_load.values()) + 1e-6) # avoid zero division
            
            # Combine start time and machine load to find the "best" machine.
            # Weight added to load_factor allows better tunning based on data
            weighted_start_time = start_time + 0.01 * load_factor 
        ...
        start_time = max(machine_time[best_machine], job_completion_time[job]) # take max to avoid race condition
