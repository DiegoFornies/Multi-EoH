...
def heuristic(input_data):
    """Combines EDD, SPT, and load balancing for FJSSP scheduling."""
    ...
    # Initial priority: EDD + SPT + Job ID
    for job in jobs:
        ops = jobs[job]
        machines, times = ops[0]
        min_time = min(times)
        priority =  min_time + (job/n_jobs)  # SPT + EDD
    ...
    while priority_list:
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = job_completion_time[job] + min_time + (job/n_jobs)
            priority_list.append((priority, job))
            priority_list.sort()
