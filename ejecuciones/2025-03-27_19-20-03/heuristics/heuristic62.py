...
def heuristic(input_data):
    """Schedules jobs, minimizes makespan, and balances load."""
    ...
    # Initial priority: shortest processing time + job id
    for job in jobs:
        ops = jobs[job]
        machines, times = ops[0]
        min_time = min(times)
        priority = min_time + (job / n_jobs)
    ...
    while priority_list:
        ...
        if best_machine is None:
            raise ValueError("No feasible machine!")
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = job_completion_time[job] + min_time + (job / n_jobs) # considers completion and job id
            priority_list.append((priority, job))
            priority_list.sort()
