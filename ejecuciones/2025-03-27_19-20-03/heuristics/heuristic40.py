...
def heuristic(input_data):
    ...
    # Initial priority: (processing_time / num_machines, job_id)
    for job in jobs:
        ops = jobs[job]
        machines, times = ops[0]
        min_time = min(times)  # Shortest possible processing time
        priority = (min_time / len(machines), job)  # Prioritize shorter operations
    ...
    while priority_list:
        ...
        if job_ops[job] < len(jobs[job]):
            ...
            priority = (min_time / len(next_machines), job)
            priority_list.append(priority)
            priority_list.sort()
