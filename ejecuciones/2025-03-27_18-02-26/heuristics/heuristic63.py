    ...
    while available_operations:
      ...
      for job, op_idx in available_operations:
        ...
        for m_idx, m in enumerate(machines):
          ...
          weighted_start_time = start_time + machine_load[m]
          if weighted_start_time < min_start_time:
            min_start_time = weighted_start_time
            best_machine = m
            best_time = times[m_idx]
            actual_start_time = max(machine_time[m], job_completion_time[job])

        if min_start_time < earliest_start:
          earliest_start = min_start_time
          best_op = (job, op_idx, best_machine, best_time, actual_start_time)
