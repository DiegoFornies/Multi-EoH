    ...
    for job_id, operations in jobs_data.items():
        ...
        for op_idx, op_data in enumerate(operations):
            ...
            for machine_idx, machine in enumerate(machines):
                machine = machines[machine_idx] # Ensure machine id is correct
                start_time = max(machine_available_times[machine], job_completion_times[job_id])
                makespan = start_time + processing_times[machine_idx] #+ machine_load[machine] Consider only processing time for makespan calculation
            ...
            machine_load[best_machine] = end_time #Update with end time
