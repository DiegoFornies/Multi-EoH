...
def heuristic(input_data):
    """A heuristic that balances makespan, idle time, and machine load."""
    ...
    while available_operations:
        ...
        for job, op_idx in available_operations:
            ...
            # Find the machine that minimizes processing time while considering available time
            best_machine = None
            ...
            for m_idx, m in enumerate(machines):
              ...
              if times[m_idx] < min_processing_time :
                min_processing_time = times[m_idx]
                best_machine = m
                best_time = times[m_idx]
                best_start_time = start_time
              elif times[m_idx] == min_processing_time and start_time < best_start_time:
                best_machine = m
                best_time = times[m_idx]
                best_start_time = start_time
           
            if min_processing_time < shortest_processing_time:
                shortest_processing_time = min_processing_time
                best_op = (job, op_idx, best_machine, best_time, best_start_time)
