...
def heuristic(input_data):
    """Schedules jobs using a combination of EFT and machine load balancing."""
    ...
    while ready_operations:
        ...
        # Machine selection based on completion time and machine load
        for machine_id, processing_time in zip(machines, times):
            available_time = machine_available_times[machine_id]
            start_time = max(available_time, job_completion_times[job_id])
            completion_time = start_time + processing_time

            # Combined metric: Completion time + weighted machine load
            workload_factor = machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6)
            combined_metric = completion_time + workload_factor * 5  # Adjusted workload weight
