...
def heuristic(input_data):
    """Schedules jobs minimizing makespan, balance, and separation."""
    ...
    while ready_operations:
        ...
        # Adaptive weight adjustment based on makespan and load
        ...
        avg_workload = total_workload / n_machines if n_machines > 0 else 0
        ...
        for m_idx, machine_id in enumerate(machines):
            ...
            balance_penalty = abs(machine_workload[machine_id] - avg_workload) / (avg_workload + 1e-6) if avg_workload > 0 else 0
            separation_benefit = 0  #Placeholder for a separation metric based on job completion times on diff machines.

            combined_score = (
                makespan_weight * makespan_delay +
                balance_weight * balance_penalty +
                separation_weight * separation_benefit +
                end_time
            )
