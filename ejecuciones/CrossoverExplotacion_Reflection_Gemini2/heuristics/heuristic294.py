...
def heuristic(input_data):
    """FJSSP heuristic: SPT, load balancing, and dynamic tie-breaking."""
    ...
    while ready_operations:
        ...
        best_weighted_finish = float('inf')
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                load_penalty = (workload / (sum(machine_load.values()) + 1e-6)) * (finish_time / (current_makespan + 1e-6)) * 5

                weighted_finish = 0.6 * finish_time + 0.4 * (machine_load[machine_id] + load_penalty)
                
                remaining_ops = len(jobs_data[job_id]) - op_index - 1
                weighted_finish += remaining_ops * 0.01

                if weighted_finish < best_weighted_finish:
                    best_weighted_finish = weighted_finish
