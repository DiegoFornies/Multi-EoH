    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                # Prioritize shorter operations and consider machine workload
                workload_factor = machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6) if sum(machine_workload.values()) > 0 else 0
                priority = end_time + workload_factor * 10  # Adjust weight as needed

                if priority < earliest_end_time:
                    earliest_end_time = priority
                    best_job = job_id
