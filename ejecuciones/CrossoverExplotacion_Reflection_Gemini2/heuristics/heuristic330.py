...
def heuristic(input_data):
    """FJSSP scheduling: Combines earliest finish and machine workload."""
    n_jobs = input_data['n_jobs']
    n_machines = input_data['n_machines']
    jobs_data = input_data['jobs']

    machine_available = {m: 0 for m in range(n_machines)}
    machine_load = {m: 0 for m in range(n_machines)} # Keep track of the total workload
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(jobs_data[job_id]):
            ...
            best_machine = None
            min_end_time_workload = float('inf') # Use machine load as secondary criteria
            ...
            for i, machine in enumerate(machines):
                ...
                workload_score = end_time + machine_load[machine]

                if end_time < min_end_time:
                    min_end_time = end_time
                    min_end_time_workload = workload_score
                    best_machine = machine
                    best_processing_time = processing_time
                elif end_time == min_end_time and workload_score < min_end_time_workload :
                    min_end_time_workload = workload_score
                    best_machine = machine
                    best_processing_time = processing_time
