...
def heuristic(input_data):
    """Schedules jobs by prioritizing separation early and balancing workload."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                # Prioritize separation early. Add machine load factor
                # Delayed assignment to create balanced schedules
                
                workload_normalized = workload / (sum(machine_workload.values()) + 1e-6)
                separation_factor = completion_time * workload_normalized * 10 # Increased impact
                
                combined_metric = completion_time + separation_factor # Main optimization is completion time
        ...
        machines, times = jobs_data[job_id][op_index]
        best_processing_time = times[machines.index(best_machine)]
