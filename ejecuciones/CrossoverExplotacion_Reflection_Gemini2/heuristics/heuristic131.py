...
def heuristic(input_data):
    """Hybrid heuristic: EFT + load balancing + dynamic tie-breaking."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                load_penalty = machine_workload[machine_id] * 0.1
                weighted_completion = completion_time + load_penalty

                remaining_ops = len(jobs_data[job_id]) - (op_index + 1)
                tie_break_value = weighted_completion + remaining_ops * 0.01
