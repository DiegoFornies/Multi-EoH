...
def heuristic(input_data):
    """Schedules jobs using a combined earliest-finish-time and workload balancing."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(jobs_data[job_id]):
            ...
            for i, machine in enumerate(machines):
                ...
                workload = machine_available[machine]

                # Combined metric: earliest end time and workload consideration
                combined_metric = end_time + (workload / (sum(machine_available.values()) + 1e-6)) * 2 # smaller workload influence
