    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for m_idx, machine_id in enumerate(machines):
                ...
                load_factor = (machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6)) * 0.2

                # Makespan factor
                makespan = max(machine_available_times.values()) if machine_available_times else 0
                makespan_factor = (end_time / (makespan + 1e-6)) * 0.3

                # Combined score
                score = end_time + load_factor + makespan_factor - separation_incentive
