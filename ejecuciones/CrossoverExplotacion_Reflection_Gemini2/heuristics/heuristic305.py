...
def heuristic(input_data):
    """Combines SPT, workload balancing, and earliest finish time for FJSSP."""
    ...
    while operations:
        ...
        for operation in available_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                # Combined metric: completion time + workload factor
                workload = machine_load[machine]
                combined_metric = completion_time + (workload / (sum(machine_load.values()) + 1e-6)) * 5  #Added workload and scaling

                remaining_ops = len([op for op in operations if op['job'] == job_id]) - 1
                tie_break_value = combined_metric + remaining_ops * 0.01 + machine_load[machine] * 0.001 # tie breaker
                
                if combined_metric < min_completion_time or (combined_metric == min_completion_time and tie_break_value < tie_breaker):
