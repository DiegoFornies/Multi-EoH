...
def heuristic(input_data):
    """FJSSP heuristic: Combines SPT, earliest start, machine load and makespan."""
    ...
    while ready_operations:
        ...
        for m_idx, machine_id in enumerate(machines):
            ...
            load_penalty = (machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6)) * 5 if sum(machine_workload.values()) > 0 else 0
            makespan_delay = (end_time / (current_makespan + 1e-6)) * 5 if current_makespan > 0 else 0
            combined_delay = load_penalty + makespan_delay

            combined_score = end_time + combined_delay + processing_time * 0.001
