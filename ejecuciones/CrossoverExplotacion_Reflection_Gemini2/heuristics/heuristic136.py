...
def heuristic(input_data):
    """Combines SPT, least loaded machine, dynamic delay, and job remaining ops."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                # Consider machine load with dynamic delay
                load_penalty = machine_load[machine_id] * delay_factor
                completion_time_with_penalty = completion_time + load_penalty

                # Dynamic tie-breaking: consider job's remaining operations and machine load
                remaining_ops = len(jobs_data[job_id]) - (op_index + 1)
                tie_break_value = completion_time_with_penalty + remaining_ops * 0.01 + machine_load[machine_id] * 0.001 #favor jobs with more operations and less loaded machine
                

                if completion_time_with_penalty < earliest_completion_time or (completion_time_with_penalty == earliest_completion_time and tie_break_value < tie_breaker):
                    earliest_completion_time = completion_time_with_penalty
        ...
        best_machine, best_processing_time = best_machine, best_processing_time
