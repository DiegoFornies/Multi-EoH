    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                # Factor in machine workload with a larger weight
                combined_metric = completion_time + (workload / (sum(machine_workload.values()) + 1e-6)) * 3

                # Prioritize operations with shorter processing times within jobs
                combined_metric += processing_time * 0.001
