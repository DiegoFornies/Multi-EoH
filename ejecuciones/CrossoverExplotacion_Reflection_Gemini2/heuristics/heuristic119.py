...
def heuristic(input_data):
    ...
    while ready_operations:
        ...
        # Prioritize operations with fewer machine choices to reduce later bottlenecks
        num_available_machines = float('inf')
        
        for job_id, op_index in ready_operations:
            machines, times = jobs_data[job_id][op_index]

            if len(machines) < num_available_machines:
                num_available_machines = len(machines)
                best_job, best_op_index = job_id, op_index
            elif len(machines) == num_available_machines:
        ...
        for machine_id, processing_time in zip(machines, times):
            ...
            workload_scaling = 5 if sum(machine_workload.values()) > 0 else 1
            combined_metric = completion_time + (workload / (sum(machine_workload.values()) + 1e-6)) * workload_scaling
