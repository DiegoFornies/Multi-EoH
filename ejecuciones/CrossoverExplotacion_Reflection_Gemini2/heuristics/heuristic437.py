...
def heuristic(input_data):
    """Combines EFT, SPT, dynamic machine load, and job priority for FJSSP."""
    ...
    job_priority = {j: 0 for j in range(1, n_jobs + 1)}  # Track job priority
    ...
    while operations:
        ...
        if not eligible_operations:
            # If no operation is immediately feasible, select the shortest operation.
            ...
            for i, machine in enumerate(machines):
                ...
                weighted_finish = 0.6 * finish_time + 0.2 * machine_load[machine] + 0.2 * times[i] #SPT

                load_penalty = machine_load[machine] * delay_factor
                combined_score = weighted_finish + load_penalty + machine * 0.0001 + job_priority[job_id]*0.00001
