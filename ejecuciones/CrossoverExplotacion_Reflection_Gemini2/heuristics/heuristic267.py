...
def heuristic(input_data):
    """Combines EFT, load balancing, and dynamic job prioritization."""
    ...
    while operations:
        ...
        if not eligible_operations:
            ...
            #Dynamic delay factor based on job completion and machine load
            ...
            for i, machine in enumerate(machines):
                ...
                # Prioritize machines with lower load. Also adding the machine ID as a tiebreaker
                combined_score = machine_load[machine] + end_time + load_penalty + machine * 0.0001
        ...
        for operation in eligible_operations:
            ...
            # Normalize workload and processing time
            max_workload = max(machine_load.values()) + 1e-6
            max_processing_time = max(times) + 1e-6  # Avoid zero division if max_processing_time is initially 0

            for i, machine in enumerate(machines):
                start_time = max(machine_available_time[machine], job_last_end_time[job_id])
                completion_time = start_time + times[i]
                
                normalized_workload = machine_load[machine] / max_workload
                normalized_processing_time = times[i] / max_processing_time

                # Combining machine load, job completion time and normalized metrics for scoring
                weighted_finish = 0.6 * completion_time + 0.2 * machine_load[machine] + 0.1 * job_last_end_time[job_id] + 0.1 * normalized_workload
                
                if weighted_finish < min_combined_score:
                    min_combined_score = weighted_finish
