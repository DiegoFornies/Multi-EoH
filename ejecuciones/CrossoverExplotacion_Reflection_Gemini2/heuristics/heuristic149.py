
def heuristic(input_data):
    """Combines SPT, least loaded machine, and dynamic prioritization."""
    n_jobs = input_data['n_jobs']
    n_machines = input_data['n_machines']
    jobs_data = input_data['jobs']

    machine_available = {m: 0 for m in range(n_machines)}
    job_completion = {j: 0 for j in range(1, n_jobs + 1)}
    machine_load = {m: 0 for m in range(n_machines)}
    schedule = {}
    operations = []

    for job_id in range(1, n_jobs + 1):
        if job_id not in schedule:
            schedule[job_id] = []
        for op_idx, operation in enumerate(jobs_data[job_id]):
            operations.append({
                'job': job_id,
                'operation': op_idx + 1,
                'machines': operation[0],
                'processing_times': operation[1]
            })

    while operations:
        eligible_operations = []
        for op in operations:
            if op['operation'] == len(schedule[op['job']]) + 1:
                eligible_operations.append(op)

        if not eligible_operations:
            shortest_operation = min(operations, key=lambda op: min(op['processing_times']))
            eligible_operations = [shortest_operation]
            #break

        best_operation = None
        best_machine = None
        min_weighted_end_time = float('inf')
        best_processing_time = None

        for operation in eligible_operations:
            job_id = operation['job']
            machines = operation['machines']
            processing_times = operation['processing_times']

            for i, machine in enumerate(machines):
                processing_time = processing_times[i]
                start_time = max(machine_available[machine], job_completion[job_id])
                end_time = start_time + processing_time
                weighted_end_time = end_time + machine_load[machine]

                if weighted_end_time < min_weighted_end_time:
                    min_weighted_end_time = weighted_end_time
                    best_machine = machine
                    best_processing_time = processing_time
                    best_operation = operation

        if best_operation is None:
            break

        job_id = best_operation['job']
        op_idx = best_operation['operation']

        start_time = max(machine_available[best_machine], job_completion[job_id])
        end_time = start_time + best_processing_time

        schedule[job_id].append({
            'Operation': op_idx,
            'Assigned Machine': best_machine,
            'Start Time': start_time,
            'End Time': end_time,
            'Processing Time': best_processing_time
        })

        machine_available[best_machine] = end_time
        job_completion[job_id] = end_time
        machine_load[best_machine] += best_processing_time
        operations.remove(best_operation)

    return schedule
