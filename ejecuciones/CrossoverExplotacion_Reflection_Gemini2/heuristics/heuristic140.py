...
def heuristic(input_data):
    """Schedules jobs balancing makespan and separation.
    Prioritizes separation early, potentially delaying assignments.
    """
    ...
    while ready_operations:
        ...
        # Select machine considering workload and potential delays for separation
        best_machine, best_processing_time = None, float('inf')
        earliest_completion_time = float('inf')

        # Calculate average workload for separation consideration
        avg_workload = sum(machine_workload.values()) / (n_machines + 1e-6)
        ...
        for machine_id, processing_time in zip(machines, times):
            ...
            # Separation factor: Encourages use of less loaded machines
            separation_factor = max(0, avg_workload - workload) * 2  # Increased separation factor

            # Combine completion time, workload, and separation factor
            combined_metric = completion_time + (workload / (sum(machine_workload.values()) + 1e-6)) * 5 - separation_factor
        ...
        # Introduce potential delay to improve separation for subsequent jobs
        delay = 0
        next_ops = []
        for job, op_idx in ready_operations:
          if op_idx == op_index+1 and job == job_id:
            next_ops.append(job)

        if len(next_ops) > 0:
          delay = min(5, best_processing_time) # adding small delay to improve separation

        end_time = start_time + best_processing_time + delay
        ...
        schedule[job_id].append({
            ...
            'Start Time': start_time - delay,
            'End Time': end_time - delay,
