...
def heuristic(input_data):
    """Schedules jobs, balances workload, and reduces idle time."""
    ...
    while ready_operations:
        ...
        job_remaining_operations = sum(len(jobs_data[j]) - len(schedule[j]) for j in range(1, n_jobs+1))
        ...
        for m_idx, machine_id in enumerate(machines):
            ...
            # Combined workload, makespan delay and job urgency.
            ...
            job_urgency = (len(jobs_data[job_id]) - op_index) / job_remaining_operations * 5 # Jobs with more remaining operations have higher urgency.
            combined_delay = load_penalty + makespan_delay + job_urgency

            combined_score = end_time + combined_delay + machine_workload[machine_id] * delay_factor #Incorporating variance
            
            # Introduce a reward for reducing idle time
            idle_reward = 0
            if start_time > machine_available_times[machine_id]: #idle time is introduced.
                idle_reward = -(start_time - machine_available_times[machine_id]) * 0.5

            combined_score += idle_reward
