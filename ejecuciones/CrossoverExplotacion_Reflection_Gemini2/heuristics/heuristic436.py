...
def heuristic(input_data):
    """Combines EFT, machine load, and operation priority."""
    ...
    while operations:
        ...
        if not eligible_operations:
            ...
            total_load = sum(machine_load.values()) + 1e-6  # Avoid division by zero
            for i, machine in enumerate(machines):
                start_time = max(machine_available_time[machine], job_last_end_time[job_id])
                finish_time = start_time + times[i]

                workload_factor = (machine_load[machine] / total_load) * 5  # Scale workload
                combined_score = finish_time + workload_factor  # balance
        ...
        for operation in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                workload_factor = (machine_load[machine] / total_load) * 5  # Scale workload
                combined_score = finish_time + workload_factor
