...
def heuristic(input_data):
    """Schedules jobs considering makespan, load, and job completion."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(jobs_data[job_id]):
            ...
            for i, machine in enumerate(machines):
                ...
                load_penalty = machine_load[machine] * 0.0001  # Small penalty for load

                # Combined metric: Earliest start with a weighted load
                combined_metric = start_time + load_penalty
                
                if combined_metric < best_start_time:
                    best_start_time = combined_metric
