...
def heuristic(input_data):
    """Schedules jobs using SPT, earliest start, machine load, and job completion."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(jobs_data[job_id]):
            ...
            for i, machine in enumerate(machines):
                ...
                load_penalty = machine_load[machine] * 0.0001  # Small penalty
                completion_time_with_penalty = end_time + load_penalty

                if (start_time + load_penalty < best_start_time) or \
                   (start_time + load_penalty == best_start_time and processing_time < best_processing_time if best_processing_time is not None else True):
                    
                    best_start_time = start_time + load_penalty
                    min_end_time = end_time #remove the processing_time
