...
def heuristic(input_data):
    """Combines SPT & machine load balancing with job dependencies."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                delay_factor = (workload / (sum(machine_load.values()) + 1e-6)) * (finish_time / (current_makespan + 1e-6)) * 5

                weighted_finish = 0.6 * finish_time + 0.4 * (machine_load[machine_id] + delay_factor)
