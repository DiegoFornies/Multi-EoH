...
def heuristic(input_data):
    """Combines earliest finish time, SPT, and machine idle time."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(jobs_data[job_id]):
            ...
            max_idle_time = -1 # Initialize max_idle_time

            for i, machine in enumerate(machines):
                processing_time = processing_times[i]
                start_time = max(machine_available[machine], job_completion[job_id])
                end_time = start_time + processing_time
                idle_time = start_time - machine_available[machine] if start_time > machine_available[machine] else 0
                ...
                if end_time < min_end_time:
                    ...
                    if processing_time < min_processing_time:
                        ...
                        best_processing_time = processing_time
                        max_idle_time = idle_time
                    elif processing_time == min_processing_time and idle_time > max_idle_time: #maximize idle_time if other metrics are equal.
                        best_machine = machine
                        best_processing_time = processing_time
                        max_idle_time = idle_time
