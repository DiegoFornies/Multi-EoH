...
def heuristic(input_data):
    """Combines SPT, least loaded machine, and dynamic delay for FJSSP."""
    ...
    while ready_operations:
        ...
        #find the best operation among the candidates
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                delay_factor = (workload / (sum(machine_workload.values()) + 1e-6)) * (completion_time / (current_makespan + 1e-6)) * 2 if current_makespan > 0 else 0

                combined_metric = completion_time + (workload / (sum(machine_workload.values()) + 1e-6)) * 0.5 + delay_factor
