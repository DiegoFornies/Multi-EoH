...
def heuristic(input_data):
    """Schedules jobs prioritizing SPT, least loaded machine, and dynamic tie-breaking."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                # SPT and least loaded machine
                combined_metric = completion_time + (workload / (sum(machine_workload.values()) + 1e-6))

                # Tie-breaking: favor shorter operations, then more remaining ops
                remaining_ops = len(jobs_data[job_id]) - (op_index + 1)
                tie_break_value = processing_time + remaining_ops * 0.001

                if combined_metric < earliest_completion_time or \
                   (combined_metric == earliest_completion_time and tie_break_value < tie_breaker):
