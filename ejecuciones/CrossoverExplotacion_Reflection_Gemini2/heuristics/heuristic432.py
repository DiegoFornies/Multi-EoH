...
def heuristic(input_data):
    """Minimizes makespan, balances load. Hybrid approach."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for m_idx, machine_id in enumerate(machines):
                ...
                # Heuristic function with makespan and balance
                makespan_delay = (end_time / (max(machine_available_times.values()) if machine_available_times.values() else 1)) * 0.2
                load_penalty = (machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6)) * 0.2
                combined_score = end_time + makespan_delay + load_penalty + processing_time*0.01
        ...
        best_machine = machines[machines.index(best_machine)] if best_machine in machines else machines[0]
        machines, times = jobs_data[job_id][op_index]
