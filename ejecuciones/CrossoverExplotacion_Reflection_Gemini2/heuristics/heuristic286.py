    ...
    while operations:
        ...
        for operation in available_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                weighted_completion = 0.7 * completion_time + 0.3 * load_penalty #Weighting finish time and load
                remaining_ops = len([op for op in operations if op['job'] == job_id]) - 1
                tie_break_value = weighted_completion + remaining_ops * 0.01 + machine_load[machine] * 0.001 # tie breaker
                if weighted_completion < min_completion_time or (weighted_completion == min_completion_time and tie_break_value < tie_breaker):
