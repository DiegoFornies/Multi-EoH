...
def heuristic(input_data):
    """Combines workload balancing & makespan reduction."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for m_idx, machine_id in enumerate(machines):
                ...
                workload_penalty = (machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6)) * 5 #workload percentage
                current_makespan = max(machine_available_times.values()) if machine_available_times else 0
                makespan_delay = (end_time / (current_makespan + 1e-6)) * 3 # makespan effect

                score = end_time + workload_penalty + makespan_delay #Combined effect
                if score < min_score:
                    min_score = score
