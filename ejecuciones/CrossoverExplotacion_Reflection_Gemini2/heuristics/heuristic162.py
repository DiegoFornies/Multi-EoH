...
def heuristic(input_data):
    ...
    while ready_operations:
        ...
        if load_values:
            ...
            delay_factor = 0.01 * load_variance  # Scale to Variance
        else:
            delay_factor = 0.01
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                load_penalty = machine_load[machine_id] * delay_factor  # Scale to workload
                weighted_completion = completion_time + load_penalty

                # Tie break : Consider remaining operations and workload for dynamic decisions
                remaining_ops = len(jobs_data[job_id]) - (op_index + 1)
                tie_break_value = weighted_completion + remaining_ops * 0.01 + machine_load[machine_id] * 0.001
