...
def heuristic(input_data):
    """Schedules jobs considering makespan, machine workload, and job separation."""
    ...
    while ready_operations:
        ...
        #Dynamically adjust separation based on remaining operations
        remaining_operations = sum([len(jobs_data[job_id]) - op_index -1 for job_id, op_index in ready_operations])
        separation_weight = 0.05 * (1 + remaining_operations / (n_jobs * sum([len(job) for job in jobs_data.values()]) + 1e-6))
