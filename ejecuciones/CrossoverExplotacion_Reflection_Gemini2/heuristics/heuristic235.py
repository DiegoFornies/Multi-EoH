...
def heuristic(input_data):
    """
    Combines earliest start time, machine workload, and job dependencies
    to schedule jobs, minimizing makespan and balancing machine load.
    """
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                # Combine finish time with machine workload
                workload_factor = (machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6)) * 5
                combined_metric = finish_time + workload_factor

                if combined_metric < earliest_finish_time:
                    earliest_finish_time = combined_metric
                    best_job, best_op_index = job_id, op_index
                    best_machine, best_processing_time = machine_id, processing_time
                elif combined_metric == earliest_finish_time and processing_time < best_processing_time:
                    best_job, best_op_index = job_id, op_index
                    best_machine, best_processing_time = machine_id, processing_time
