...
def heuristic(input_data):
    """Combines EFT, SPT, and dynamic prioritization for FJSSP."""
    ...
    while operations:
        ...
        if not eligible_operations:
            #If No operation are eligible, find an operation that can be schedule
            ...
            if load_values:
                load_variance = sum([(x - sum(load_values) / len(load_values)) ** 2 for x in load_values]) / len(load_values) if len(load_values) > 0 else 0
                delay_factor = 0.01 * load_variance
            else:
                delay_factor = 0.01

            # Normalize workload and processing time
            max_workload = max(machine_load.values()) + 1e-6 if machine_load else 1e-6
        ...
        for operation in eligible_operations:
            ...
            max_workload = max(machine_load.values()) + 1e-6 if machine_load else 1e-6
            max_processing_time = max(times) + 1e-6
