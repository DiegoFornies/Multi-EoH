...
def heuristic(input_data):
    ...
    while ready_operations:
        ...
        # Introduce a delay factor AND workload balancing.
        load_values = list(machine_workload.values())
        avg_load = sum(load_values) / len(load_values) if load_values else 0
        load_variance = sum([(x - avg_load) ** 2 for x in load_values]) / len(load_values) if load_values else 0
        ...
        for m_idx, machine_id in enumerate(machines):
            ...
            combined_metric = completion_time + (workload + delay_factor)
