...
def heuristic(input_data):
    """
    A heuristic algorithm for FJSSP that prioritizes minimizing idle time
    between operations within the same job and balancing machine load.
    """
    ...
    # Maintain a list of available operations for each job. Initially, it's the first operation.
    available_operations = {job_id: 0 for job_id in jobs_data.keys()} 

    # Use a list of operations to schedule by priority.
    operations_to_schedule = []
    for job_id, job_data in jobs_data.items():
        machines, times = job_data[0]
        operations_to_schedule.append((job_id, 0))

    while operations_to_schedule:
        # Sort operations by earliest possible start time (job completion time or machine availabilty)
        ...
        # Find the machine that allows for the earliest completion time considering job and machine availability
        best_machine = None
        ...
        # Add next operation of this job to the list of operations to schedule
