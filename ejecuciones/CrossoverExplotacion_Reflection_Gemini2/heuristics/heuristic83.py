...
def heuristic(input_data):
    """Combines workload awareness with dynamic tie-breaking for FJSSP."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                combined_metric = completion_time + (workload / (sum(machine_workload.values()) + 1e-6)) * 2

                # Dynamic tie-breaking: consider job's remaining operations
                remaining_ops = len(jobs_data[job_id]) - (op_index + 1)
                tie_break_value = combined_metric + remaining_ops * 0.01 #favor jobs with more operations
