...
def heuristic(input_data):
    """Minimizes makespan, improves separation using load-aware dynamic delay."""
    ...
    while ready_operations:
        ...
        for machine_id, processing_time in zip(machines, times):
            ...
            # Dynamic delay factor based on workload and makespan
            current_makespan = max(machine_available_times.values()) + 1e-6
            workload_ratio = workload / total_workload
            delay_factor = workload_ratio * (completion_time / current_makespan) * 5 # Increased scaling
            
            # Combined metric: completion time + normalized workload + delay
            combined_metric = completion_time + workload_ratio * 5 + delay_factor
