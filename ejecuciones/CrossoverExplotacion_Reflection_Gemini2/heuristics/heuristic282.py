...
def heuristic(input_data):
    """Combines EFT, SPT, load balancing, and job completion time."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(jobs_data[job_id]):
            ...
            for i, machine in enumerate(machines):
                ...
                # Combine EFT, SPT, machine load, and job completion
                weighted_finish = 0.6 * end_time + 0.4 * processing_time
                load_penalty = 0.005 * machine_available[machine]
                job_penalty = 0.005 * job_completion[job_id] #prioritizing jobs that are already late
                combined_score = weighted_finish + load_penalty + job_penalty
