...
def heuristic(input_data):
    """Combines SPT, workload balancing, and dynamic tie-breaking for FJSSP."""
    ...
    while operations:
        ...
        current_makespan = max(machine_available_time.values()) if machine_available_time else 0
        ...
        for operation in available_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                # Combined workload and makespan aware delays
                ...
                combined_score = end_time + combined_delay + machine_load[machine] * delay_factor

                remaining_ops = len([op for op in operations if op['job'] == job_id]) - 1
                tie_break_value = combined_score + remaining_ops * 0.01

                if combined_score < min_combined_score or (combined_score == min_combined_score and tie_break_value < tie_breaker):
