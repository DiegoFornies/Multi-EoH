...
def heuristic(input_data):
    ...
    while ready_operations:
        ...
        separation_factor = 0.1  # Adjust this factor to influence separation
        ...
        for machine_id, processing_time in zip(machines, times):
            ...
            # Penalize machines used recently by OTHER jobs to improve separation.
            recent_usage_penalty = 0
            for j in range(1, n_jobs + 1):
                if j != job_id and schedule[j]:
                    last_op = schedule[j][-1]
                    if last_op['Assigned Machine'] == machine_id:
                        recent_usage_penalty = max(0, last_op['End Time'] - start_time)  # Larger penalty if ending time is closer to start time
            # Prioritize less loaded machines with smaller completion times and considers separation.
            combined_metric = completion_time + (workload / (sum(machine_workload.values()) + 1e-6)) * 5 + recent_usage_penalty * separation_factor
