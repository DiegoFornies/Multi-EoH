...
def heuristic(input_data):
    """Schedules jobs adaptively, balancing makespan, separation, and machine load."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for m_idx, machine_id in enumerate(machines):
                ...
                # Adaptive weighting based on workload and makespan
                load_penalty = (machine_workload[machine_id] / (sum(machine_workload.values()) + 1e-6)) * 5
                makespan_delay = (end_time / (max(machine_available_times.values()) if machine_available_times.values() else 1)) * 5
                combined_delay = load_penalty + makespan_delay

                combined_score = end_time + combined_delay # Simplification: Remove variance
