...
def heuristic(input_data):
    """Combines EFT, load balancing, and dynamic tie-breaking."""
    ...
    while ready_operations:
        ...
        for job_id, op_index in ready_operations:
            ...
            for machine_id, processing_time in zip(machines, times):
                ...
                workload_penalty = machine_workload[machine_id] * delay_factor
                weighted_completion = completion_time + workload_penalty

                remaining_ops = len(jobs_data[job_id]) - (op_index + 1)
                tie_break_value = weighted_completion + remaining_ops * 0.01 + machine_workload[machine_id] * 0.001

                if weighted_completion < min_completion_time or \
                   (weighted_completion == min_completion_time and tie_break_value < tie_breaker):
                    min_completion_time = weighted_completion
                    best_job, best_op_index = job_id, op_index
                    best_machine = machine_id
                    best_processing_time = processing_time #store for scheduling
