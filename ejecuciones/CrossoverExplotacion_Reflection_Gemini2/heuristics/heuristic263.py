...
def heuristic(input_data):
    """Combines EFT, SPT, and dynamic workload balancing for FJSSP."""
    ...
    while operations:
        ...
        if not eligible_operations:
            ...
            if load_values:
                load_variance = sum([(x - sum(load_values) / len(load_values)) ** 2 for x in load_values]) / (len(load_values)+1e-9)
