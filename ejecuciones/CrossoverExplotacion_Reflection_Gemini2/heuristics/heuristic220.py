...
def heuristic(input_data):
    """Schedules jobs considering makespan, workload, and operation separation."""
    ...
    while ready_operations:
        ...
        # Select machine considering makespan, workload, and operation separation.
        ...
        average_workload = sum(machine_workload.values()) / (n_machines + 1e-6)
        ...
        for m_idx, machine_id in enumerate(machines):
            ...
            #Prioritize machines with workload closest to average, avoid bottleneck
            workload_diff = abs(machine_workload[machine_id] - average_workload)
            workload_penalty = workload_diff #Higher difference higher the penalty

            #Avoid schedule an operation in the critical path if possible.
            makespan_penalty = end_time / (current_makespan + 1e-6) if current_makespan > 0 else 0 #If increases the makespan adds a penalty.
            combined_score = end_time + workload_penalty + makespan_penalty
