...
def heuristic(input_data):
    """SPT + Machine Load Balance + Dynamic Priority."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            job_remaining_time = sum(min(op[1]) for i, op in enumerate(jobs[job_id]) if i > operation_index)
            ...
            for i in range(len(possible_machines)):
                ...
                weighted_time = end_time + 0.05 * machine_load[machine] - 0.01 * job_remaining_time  #Balance Time and remaining tasks
