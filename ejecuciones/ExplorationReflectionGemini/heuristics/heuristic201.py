...
def heuristic(input_data):
    ...
    while available_operations:
        ...
        for op_data in available_operations:
            ...
            # Adaptive Weighting based on machine load
            load_factor = machine_load / sum(machine_available_times.values()) if sum(machine_available_times.values()) > 0 else 0
            spt_weight = 0.8 - 0.3 * load_factor #Favor SPT more
            load_weight = 0.2 + 0.3 * load_factor

            score = spt_weight * (earliest_start_time + best_time) + load_weight * machine_load
