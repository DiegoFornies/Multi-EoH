...
def heuristic(input_data):
    """Combines earliest start, machine load, and job urgency dynamically."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                # Adaptive weighting based on remaining time and machine load.
                urgency_factor = 0.1 * (job_remaining_times[job_id] / (sum(job_remaining_times.values()) + 1e-6))
                load_factor = 0.05 * (machine_load[machine] / (sum(machine_load.values()) + 1e-6))
                weighted_time = end_time + load_factor - urgency_factor
