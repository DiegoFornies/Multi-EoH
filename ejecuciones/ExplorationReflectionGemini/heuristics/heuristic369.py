...
def heuristic(input_data):
    """Earliest start time, SPT, and proactive load balancing."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                # Combine completion time, machine load (proactive), and processing time
                weighted_time = end_time + 0.1 * machine_load[machine] + 0.05 * processing_time
                #weighted_time = start_time + 0.8 * processing_time + 0.2 * machine_load[machine]
