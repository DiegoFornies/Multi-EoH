...
def heuristic(input_data):
    """
    Combines SPT, earliest avail., and machine preference based on workload.
    Dynamically adjusts weights for SPT vs. load balancing.
    """
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                job_progress = job_completion_times[job_id] / sum([sum(times) for ops in jobs.values() for times in [op[1] for op in ops]])
                weight = 0.3 + 0.4 * load_factor + 0.3 * job_progress # Favor SPT for urgent jobs, balance with machine load.

                weighted_time = end_time * weight + processing_time * (1-weight)
