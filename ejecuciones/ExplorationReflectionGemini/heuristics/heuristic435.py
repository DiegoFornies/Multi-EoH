...
def heuristic(input_data):
    ...
    # Create a list of all operations with job ID and operation index
    operations = []
    for job_id in range(1, n_jobs + 1):
        for operation_index, operation_data in enumerate(jobs[job_id]):
            operations.append((job_id, operation_index))

    # Sort operations by the number of possible machines in ascending order
    ...
    for job_id, operation_index in operations:
        ...
        # Choose the machine that minimizes the completion time of the operation
        best_machine = None
