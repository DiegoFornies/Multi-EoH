...
def heuristic(input_data):
    """Combines SPT, earliest availability, and adaptive load balancing."""
    ...
    for job in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(jobs[job]):
            ...
            for i, machine in enumerate(machines):
                ...
                # SPT, Earliest Availability, Load Balance
                # Adaptive load balancing: less impact for smaller jobs
                load_balance_factor = machine_load[machine] * (1 - (processing_time / sum([sum(t) for _, t in jobs[job]])))
                combined_metric = completion_time + 0.1 * load_balance_factor + 0.01 * processing_time
