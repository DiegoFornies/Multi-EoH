...
def heuristic(input_data):
    """Schedules operations considering both job urgency and machine load."""
    ...
    schedule = {}
    job_priority = {j:0 for j in range(1, n_jobs + 1)}
    
    for job_id in range(1, n_jobs + 1):
        schedule[job_id] = []

    uncompleted_jobs = set(range(1, n_jobs+1))

    while uncompleted_jobs:
        #Prioritize job with shortest remaining processing time
        current_job = None
        ...
        for job_id in uncompleted_jobs:
          ...
          for i in range(scheduled_operations, len(job_operations)):
            remaining_time+=min(job_operations[i][1]) #Shortest processing time
        ...
        # Schedule on the machine that minimizes completion time + machine load.
        best_machine = None
        ...
        for i in range(len(possible_machines)):
            ...
            weighted_time = end_time + 0.2 * machine_load[machine] #Adjust machine load influence
