...
def heuristic(input_data):
    """Combines earliest start time with dynamic machine load balancing and SPT job prioritization."""
    ...
    job_priority = sorted(list(range(1, n_jobs + 1)), key=lambda x: job_remaining_times[x])
    ...
    for job_id in job_priority:
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                weighted_time = end_time + 0.1 * machine_load[machine] # Machine load aware
