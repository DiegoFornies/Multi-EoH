...
def heuristic(input_data):
    """Combines SPT, load balancing, and job urgency for FJSSP scheduling."""
    ...
    while available_operations:
        ...
        for op_data in available_operations:
            ...
            job_urgency = remaining_ops

            for machine_idx, (machine, time) in enumerate(zip(machines, times)):
                start_time = max(machine_available_times[machine], job_completion_times[job_id])
                end_time = start_time + time

                avg_load = sum(machine_load.values()) / n_machines if n_machines else 0
                load_weight = 0.1 if machine_load[machine] <= avg_load else 0.5

                cost = time + load_weight * machine_load[machine] - 0.05 * job_urgency + 0.01 * start_time
