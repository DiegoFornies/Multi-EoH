...
def heuristic(input_data):
    """FJSSP heuristic: Combines SPT, load balancing, and job urgency for makespan and balance."""
    ...
    while any(job_operations_scheduled[job] < len(jobs[job]) for job in jobs):
        ...
        for job in jobs:
            ...
            if next_op_index < len(jobs[job]):
                available_operations.append((job, next_op_index))
        
        # Prioritize operations based on job urgency (remaining time)
        available_operations.sort(key=lambda x: job_remaining_times[x[0]])

        for job, next_op_index in available_operations:
            ...
            for i, m in enumerate(machines):
                ...
                weighted_time = start_time + times[i] + 0.05 * machine_load[m]  # Load balancing
            ...
            if best_machine is not None:
                ...
                job_remaining_times[job] -= processing_time  # Update remaining time
