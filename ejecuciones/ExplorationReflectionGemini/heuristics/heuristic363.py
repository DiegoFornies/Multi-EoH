...
def heuristic(input_data):
    """Hybrid heuristic: SPT for job order, load balancing for machine selection."""
    ...
    while any(job_operations_scheduled[job] < len(jobs[job]) for job in jobs):
        for job in available_jobs:
            ...
            for i, m in enumerate(machines):
                ...
                end_time_candidate = start_time + times[i]
                load_balance_score = end_time_candidate
                if load_balance_score < best_time:
                    best_time = load_balance_score
                    best_machine = m
