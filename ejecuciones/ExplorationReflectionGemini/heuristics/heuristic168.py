...
def heuristic(input_data):
    """Combines earliest start time, SPT, and dynamic machine load for FJSSP."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                # Dynamic weight based on machine load and operation time
                load_factor = machine_load[machine] / (sum(machine_load.values()) + 1e-9)
                time_factor = processing_time / (sum(possible_times) + 1e-9) #SPT
                weighted_time = start_time + 0.5*processing_time+ 0.5 * machine_load[machine]
