...
def heuristic(input_data):
    """Schedules jobs balancing makespan and machine load, using a dynamic priority rule."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            # Use a weight combining completion time and machine load, and urgent job
            for machine_index, machine in enumerate(possible_machines):
                ...
                #Dynamic Weighting function combining machine load, job urgency, and operation slack
                slack = job_remaining_times[job_id] - processing_time
                weighted_time = end_time + 0.05 * machine_time[machine] - 0.1 * job_remaining_times[job_id] + 0.01 * slack
