...
def heuristic(input_data):
    """Combines SPT on least loaded machine with earliest start time."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for machine_index, machine in enumerate(possible_machines):
                ...
                # Calculate start time considering machine availability and job completion
                start_time = max(machine_available_times[machine], job_completion_times[job_id])
                
                #Prioritize machines with low load, break ties using SPT and start time
                load_penalty = machine_load[machine] * 0.01
                adjusted_start_time = start_time + load_penalty + processing_time * 0.01
