...
def heuristic(input_data):
    """FJSSP heuristic: Combines earliest availability, SPT, and dynamic load balancing."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                # Dynamic load balancing: Adjust weight based on machine load
                load_factor = machine_load[machine] / (sum(machine_load.values()) + 1e-9)  # Normalize load
                weighted_time = end_time + 0.1 * load_factor * processing_time # + SPT Prioritization
