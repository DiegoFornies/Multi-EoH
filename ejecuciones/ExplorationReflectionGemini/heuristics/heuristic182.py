...
def heuristic(input_data):
    """Combines earliest start, SPT, and adaptive machine load."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                # Adaptive Weighting: More load -> less influence of machine load
                load_factor = 1 / (1 + machine_load[machine])  # Reduce weight as load increases
                weighted_time = start_time + 0.7 * processing_time + 0.3 * load_factor * machine_load[machine]
