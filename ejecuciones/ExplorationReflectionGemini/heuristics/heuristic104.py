    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                # Dynamic load balancing weight based on machine load
                load_weight = machine_load[machine] / sum(machine_load.values()) if sum(machine_load.values()) > 0 else 0
                weighted_time = end_time + 0.5 * load_weight * processing_time # Adjusted weight
