...
def heuristic(input_data):
    """FJSSP heuristic: SPT, load balancing, job criticality, and lookahead."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i, machine in enumerate(possible_machines):
                ...
                # Cost: SPT + load balance + job criticality + waiting time
                load_weight = 0.5 # Increased load balancing weight
                cost = processing_time + load_weight * machine_load[machine] - 0.05 * job_criticality + 0.01 * start_time
                
                # Check future load (lookahead - considering the next operation load).
                if operation_index + 1 < len(job_operations):
                   next_operation_data = job_operations[operation_index + 1]
                   if machine in next_operation_data[0]:
                        cost += 0.1 * next_operation_data[1][next_operation_data[0].index(machine)]
