...
def heuristic(input_data):
    """Combines SPT and least loaded machine, dynamically weighted."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for operation_index, operation_data in enumerate(job_operations):
            ...
            for i in range(len(possible_machines)):
                ...
                # Dynamic weight: Favor SPT when machine load is high
                load_factor = machine_load[machine] / (sum(machine_load.values()) + 1e-6) if sum(machine_load.values()) > 0 else 0
                weight = 0.5 + 0.5 * load_factor  # Adjust between 0.5 and 1.0
