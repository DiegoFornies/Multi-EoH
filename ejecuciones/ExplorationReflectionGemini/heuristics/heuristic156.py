...
def heuristic(input_data):
    """Schedules jobs, prioritizing shortest processing time, machine load."""
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_data in jobs[job_id]:
            total_time += min(op_data[1])
        job_processing_times[job_id] = total_time

    sorted_jobs = sorted(jobs.keys(), key=lambda x: job_processing_times[x])
