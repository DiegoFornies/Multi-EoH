...
def heuristic(input_data):
    """Combines job prioritization and machine selection for FJSSP."""
    ...
    # Prioritize jobs with more remaining operations initially
    ...
    while any(remaining_operations[job] for job in jobs):
        for job in job_priority:
            ...
            # Find best machine based on earliest availability, tie-break by processing time
        ...
        #Re-sort after each complete iteration
        job_priority = sorted(jobs.keys(), key=lambda job: len(remaining_operations[job]), reverse=True)
