...
def heuristic(input_data):
    """A greedy heuristic for FJSSP that prioritizes earliest available machine."""
    ...
    job_completion_time = {j: 0 for j in range(1, n_jobs + 1)}  # Completion time of each job's last operation. Start from 1
    ...
    for job_id in range(1, n_jobs + 1):
        ...
        for op_idx, operation in enumerate(job_operations):
            ...
            for i, machine in enumerate(eligible_machines):
                machine_start_time = machine_available_time[machine]
