...
def heuristic(input_data):
    ...
    for job in range(1, n_jobs + 1):
        ...
        for op_idx, (machines, times) in enumerate(jobs[job]):
            ...
            best_machine = None
            earliest_end_time = float('inf')  # Minimize operation end time.
            ...
            for m_idx, machine in enumerate(machines):
                ...
                # Prioritize earliest end time, then balance machine load.
                if end_time < earliest_end_time :
                    earliest_end_time = end_time
                    best_machine = machine
                    best_processing_time = processing_time
                elif end_time == earliest_end_time and load_factor < machine_load.get(best_machine,0):
                    best_machine = machine
                    best_processing_time = processing_time
