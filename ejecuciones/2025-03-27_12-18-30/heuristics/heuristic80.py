...
def heuristic(input_data):
    """
    Adaptive SPT and load balancing, weighting by job complexity.
    """
    ...
    while operation_queue:
        ...
        for job, op_idx in operation_queue:
            ...
            job_complexity = len(jobs[job])
            
            for i, m in enumerate(machines):
                start_time = max(machine_time[m], job_completion_time[job])
                processing_time = times[i]

                #Adaptive weighting based on job_complexity.
                weighted_start_time = start_time * (1 - (job_complexity / (n_jobs+1))) #Favor load balancing for complex jobs
                
                if weighted_start_time < best_start_time_local:
                    best_start_time_local = weighted_start_time
