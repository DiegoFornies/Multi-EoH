...
def heuristic(input_data):
    """Prioritizes critical operations on less loaded machines."""
    ...
    while any(remaining_operations.values()):
        ...
        for job, (op_num, (machines, times)) in eligible_operations:
            criticality = len(remaining_operations[job]) # Number of operations remaining

            for m_idx, m in enumerate(machines):
                processing_time = times[m_idx]
                start_time = max(machine_load[m], job_completion_time[job])
                completion_time = start_time + processing_time + criticality * 0.1 # Bias critical ops to finish faster on less loaded machines
