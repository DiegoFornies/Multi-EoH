...
def heuristic(input_data):
    """FJSSP heuristic: Combines EFT, job urgency, and load balancing."""
    ...
    while scheduled_count < total_operations:
        ...
        #Minimizes makespan using EFT
        for machine, time in zip(machines, times):
            start_time = max(machine_available_time[machine], job_completion_time[urgent_job] if op_index > 0 and len(schedule[urgent_job]) > 0 else 0)
        ...
        # Load balancing adjustment (move after EFT)
        ...
        for machine, time in zip(machines, times):
            ...
            if (machine_load[best_machine] > machine_load[machine]):
                if finish_time < best_start_time + 1.1*best_processing_time : # add a threshold to control balance effect
                    best_machine = machine
                    best_start_time = start_time
                    best_processing_time = time
