...
def heuristic(input_data):
    """Schedules jobs minimizing makespan, load, and job separation."""
    ...
    job_machines = {j: [] for j in range(1, n_jobs + 1)}
    ...
    while scheduled_operations_count < total_operations:
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                # Job separation: penalize machine if recently used by same job family
                separation_penalty = 0
                if job_machines[job_id] and job_machines[job_id][-1] == machine:
                    separation_penalty = 50 # arbitrary penalty. Higher the penalty, the greater the separation.

                priority = makespan_impact + 0.5*load_imbalance + separation_penalty # adjust factors as needed
        ...
        if best_operation is not None:
            ...
            job_machines[job_id].append(best_machine)
