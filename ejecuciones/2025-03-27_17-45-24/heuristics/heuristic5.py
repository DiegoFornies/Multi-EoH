    ...
    while operation_queue:
        ...
        for machine, time in zip(machines, times):
            start_time = max(machine_available_time[machine],
                             schedule[job_id][-1]['End Time'] if op_index > 0 else 0)  # respect sequence constraint
            finish_time = start_time + time

            if finish_time < best_start_time: #Minimize makespan
                best_machine = machine
                best_start_time = start_time
                best_processing_time = time

        # Load Balancing Adjustment (Minor effect)
        initial_start_time = best_start_time
        initial_machine = best_machine
        load_increase = best_processing_time
        for machine, time in zip(machines, times):
                start_time = max(machine_available_time[machine],
                                 schedule[job_id][-1]['End Time'] if op_index > 0 else 0)  # respect sequence constraint
                finish_time = start_time + time
                if (machine_load[best_machine] > machine_load[machine]): #Balances by finding an alternative
                    best_machine = machine
                    best_start_time = start_time
                    best_processing_time = time
