...
def heuristic(input_data):
    """Combines earliest finish time and machine load balancing for FJSSP."""
    ...
    while scheduled_operations_count < total_operations:
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                # Prioritize machines with lower load, but break ties with finish time
                load_factor = 0.001 * machine_loads[machine]  # Small factor to influence, not dominate

                if finish_time + load_factor < earliest_finish_time:
                    earliest_finish_time = finish_time + load_factor
