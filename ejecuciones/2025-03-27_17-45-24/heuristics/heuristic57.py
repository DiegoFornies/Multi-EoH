...
def heuristic(input_data):
    """Schedules jobs using a Shortest Processing Time (SPT) approach
    with a tie-breaking mechanism favoring balanced machine load."""
    ...
    while scheduled_operations_count < total_operations:
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                # Prioritize shortest processing time
                if processing_time < shortest_processing_time:
                    ...
                    lowest_load = machine_loads[machine] # initialize lowest_load
                elif processing_time == shortest_processing_time:
                    #Tie-breaking mechanism: favor balanced machine load
                    if machine_loads[machine] < lowest_load:
                        lowest_load = machine_loads[machine]
                        best_operation = (job_id, op_index)
                        best_machine = machine
                        earliest_start_time = start_time
