...
def heuristic(input_data):
    """Schedules jobs based on a global makespan estimation."""
    ...
    while scheduled_operations_count < total_operations:
        ...
        best_operation = None
        best_machine = None
        min_estimated_makespan = float('inf')
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                estimated_makespan = max(start_time + processing_time, max(machine_available_time.values()), max(job_completion_time.values()))
                
                if estimated_makespan < min_estimated_makespan:
                    min_estimated_makespan = estimated_makespan
