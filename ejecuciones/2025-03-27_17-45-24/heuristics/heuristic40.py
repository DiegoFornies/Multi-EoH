...
def heuristic(input_data):
    """FJSSP heuristic: Minimizes makespan, balances load, reduces idle time."""
    ...
    while scheduled_operations_count < total_operations:
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                # Prioritize machines with smaller loads
                load_factor = machine_loads[machine] / (sum(machine_loads.values()) + 1e-9)
                weighted_end_time = end_time + load_factor * 10  # Scale the load factor

                if weighted_end_time < earliest_end_time:
                    earliest_end_time = weighted_end_time
                    best_operation = (job_id, op_index)
                    best_machine = machine
                    processing_time_used = processing_time

        if best_operation is not None:
            job_id, op_index = best_operation
            start_time = max(machine_available_time[best_machine], job_completion_time[job_id])
            end_time = start_time + processing_time_used
            ...
            schedule[job_id].append({
                ...
                'End Time': end_time,
                'Processing Time': processing_time_used
            })

            machine_available_time[best_machine] = end_time
            job_completion_time[job_id] = end_time
            machine_loads[best_machine] += processing_time_used
