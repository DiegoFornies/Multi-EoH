...
def heuristic(input_data):
    """FJSSP heuristic: Dynamically balances makespan, load, and idle time using priority rules."""
    ...
    while scheduled_operations_count < total_operations:
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                # Dynamic priority based on urgency (remaining ops) and machine load
                urgency = job_remaining_ops[job_id]
                load_factor = machine_loads[machine]

                # Prioritize shorter processing times to minimize makespan
                makespan_factor = processing_time

                priority = start_time + 0.5 * urgency + 0.1 * load_factor + 0.2 * makespan_factor  # Adjusted weights

                if priority < earliest_priority:
                    earliest_priority = priority
