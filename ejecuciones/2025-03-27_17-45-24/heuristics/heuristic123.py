...
def heuristic(input_data):
    """Hybrid heuristic: EFT + load balancing for FJSSP."""
    ...
    while scheduled_operations_count < total_operations:
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                load_factor = 1 + machine_loads[machine] / (sum(machine_loads.values()) + 1e-9) if sum(machine_loads.values()) > 0 else 1 #Avoid division by zero
                adjusted_end_time = end_time * load_factor

                if adjusted_end_time < earliest_start_time:
                    earliest_start_time = adjusted_end_time  #Comparing the adjusted end time
                    best_operation = (job_id, op_index)
                    best_machine = machine
                    best_start_time_unadjusted = start_time #keep track of the unadjusted earliest start time.
                    best_processing_time = processing_time

        if best_operation is not None:
            job_id, op_index = best_operation

            start_time = best_start_time_unadjusted #Use the raw start time
