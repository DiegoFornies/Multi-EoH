...
def heuristic(input_data):
    """
    A heuristic for the FJSSP that adaptively balances makespan, idle time, and machine load.
    Prioritizes jobs dynamically based on urgency and machine availability.
    """
    ...
    while scheduled_operations_count < total_operations:
        ...
        min_job_completion_time = float('inf')
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                #Urgency factor: prioritize jobs with fewer remaining operations
                urgency = job_remaining_ops[job_id]
                
                # Load balancing: prefer machines with lower loads
                load_factor = machine_loads[machine]

                # Combination of factors for dynamic priority.
                priority = start_time + urgency + load_factor
                
                if priority < earliest_start_time:
                    earliest_start_time = priority
