...
def heuristic(input_data):
    ...
    remaining_operations = {job_id: list(range(1, len(ops) + 1))
    ...
    while scheduled_operations < total_operations:
        eligible_operations = [(job_id, remaining_operations[job_id][0] - 1)
                               for job_id in jobs if remaining_operations[job_id]]
        best_op = None
        best_machine = None
        ...
        for job_id, op_index in eligible_operations:
            ...
            for m_index, machine in enumerate(machines):
                ...
                load_factor = 1 + (machine_load[machine] /
                                   sum(machine_load.values()) if sum(machine_load.values())>0 else 0)
                weighted_finish_time = finish_time * load_factor

                if weighted_finish_time < min_finish_time:
                    min_finish_time = weighted_finish_time
