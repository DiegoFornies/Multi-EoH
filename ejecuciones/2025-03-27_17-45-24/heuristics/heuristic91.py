...
def heuristic(input_data):
    """FJSSP heuristic: Balances makespan, load, and idle time using urgency and EFT."""
    ...
    job_remaining_ops = {j: len(ops) for j, ops in jobs.items()}
    ...
    while scheduled_count < total_operations:
        ...
        # Prioritize jobs with earlier overall completion time, urgent first
        ...
        #Minimizes makespan using EFT, consider machine load for ties
        for m_index, machine in enumerate(machines):
            ...
            elif finish_time == best_start_time and machine_load[machine] < machine_load[best_machine if best_machine is not None else 0]:
                best_machine = machine
                best_start_time = start_time
                best_processing_time = time

        # Perform assignment with the machine using EFT
        operation = {
