...
def heuristic(input_data):
    """FJSSP heuristic: Combines shortest processing time, machine availability, and job completion time."""
    ...
    while len(scheduled_operations) < sum(len(ops) for ops in jobs.values()):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                # Combine machine idle time with shortes processing time
                score = start_time + processing_time  # Prioritize early start and short processing
