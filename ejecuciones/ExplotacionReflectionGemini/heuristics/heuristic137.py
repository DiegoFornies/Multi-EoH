...
def heuristic(input_data):
    """Combines SPT and load balancing for FJSSP scheduling."""
    ...
    while available_operations:
        ...
        for job_id, op_num in available_operations:
            ...
            for m_idx, m in enumerate(machines):
                start_time = max(machine_available_times[m], job_last_end_time[job_id])
                ...
                elif end_time == min_end_time and machine_load[m] < machine_load[chosen_machine] if chosen_machine is not None else False:
                    earliest_start_time = start_time
                    chosen_machine = m
                    processing_time = times[m_idx]
                    
            if chosen_machine is not None:
                best_op = (job_id, op_num, chosen_machine, processing_time, earliest_start_time)
        ...
        if best_op is not None:
            ...
            if available_operations:
                job_id, op_num = available_operations.pop(0)
                machines, times = jobs[job_id][op_num - 1]
                assigned_machine = machines[0]
                processing_time = times[0]
                start_time = max(machine_available_times[assigned_machine], job_last_end_time[job_id])
                end_time = start_time + processing_time
