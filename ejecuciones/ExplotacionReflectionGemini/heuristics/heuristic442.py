...
def heuristic(input_data):
    """
    Combines makespan minimization with balance improvement.
    Hybrid heuristic: Improves makespan initially, then iteratively balance machine load.
    """
    ...
    # Phase 1: Makespan Minimization (Greedy) - Based on Parent 1
    ...
    # Phase 2: Balance Improvement (Local Search - Reassign Machine) - Based on Parent 2 concept and Parent 1 code base
    ...
    #Iterative Balance Improvement through Reassignment of Machine for each operation
    for job_id in range(1, n_jobs+1):
      for op_idx in range(len(schedule[job_id])):
        ...
        possible_machines = jobs[job_id][op_idx][0]
        ...
        for new_machine in possible_machines:
          if new_machine != original_machine:
            ...
            processing_time = jobs[job_id][op_idx][1][jobs[job_id][op_idx][0].index(new_machine)]
