...
def heuristic(input_data):
    """Combines shortest processing time with machine reassignment to balance load and minimize makespan."""
                ...
                if new_machine != original_machine:
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            assigned_machine_re = temp_schedule[job_id_re][op_idx_re]['Assigned Machine']
                            
                            processing_time_re = times_re[machines_re.index(assigned_machine_re)]
                            start_time = max(temp_machine_available_time[assigned_machine_re], temp_job_completion_time[job_id_re])
                            end_time = start_time + processing_time_re
                            recalculated_schedule[job_id_re].append({
                                'Operation': op_idx_re + 1,
                                'Assigned Machine': assigned_machine_re,
                                'Start Time': start_time,
                                'End Time': end_time,
                                'Processing Time': processing_time_re
                            })

                            temp_machine_available_time[assigned_machine_re] = end_time
