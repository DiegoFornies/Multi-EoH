...
def heuristic(input_data):
    """Combines greedy scheduling with local search (machine swap) to minimize makespan and balance machine load."""
    ...
    while available_operations:
        best_operation = None
        best_machine = None
        earliest_end_time = float('inf') # minimize makespan
        ...
        for job_id, op_idx in available_operations:
            ...
            for machine_idx, machine in enumerate(machines):
                ...
                end_time = start_time + processing_time # makespan focused
        ...
        if len(schedule[job_id])>1:
            for i in range(len(schedule[job_id])-1):
                for j in range(i+1, len(schedule[job_id])):
                    ...
                    if new_objectives['Balance'] < original_obj['Balance'] or new_objectives['Makespan'] < original_obj['Makespan']:
                        best_schedule = schedule
