...
def heuristic(input_data):
    """Greedy makespan with SPT, local search for balance/makespan."""
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    temp_schedule = {job_id_re: [op.copy() for op in schedule[job_id_re]] for job_id_re in range(1, n_jobs + 1)}
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            if job_id_re == job_id and op_idx_re == op_idx:
                                machine = new_machine
                                processing_time = jobs[job_id_re][op_idx_re][1][machines_re.index(new_machine)]
                            else:
