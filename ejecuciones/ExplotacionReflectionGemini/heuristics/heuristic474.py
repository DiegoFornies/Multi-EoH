...
def heuristic(input_data):
    """FJSSP heuristic: Combines idle time, machine load, and remaining operations."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                idle_time = start_time - job_last_end_time[job_id] if start_time > job_last_end_time[job_id] else 0
                score = start_time + remaining + machine_load_factor + time + idle_time  # minimize idle time
