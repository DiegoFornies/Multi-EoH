...
def heuristic(input_data):
    """Hybrid: Minimizes makespan initially, then improves balance."""
    ...
    # Phase 1: Makespan Minimization (Greedy with idle time consideration)
    ...
    while available_operations:
        best_operation = None
        best_machine = None
        earliest_end_time = float('inf')  # Modified to earliest end time
        ...
        for job_id, op_idx in available_operations:
            ...
            for machine_idx, machine in enumerate(machines):
                ...
                if end_time < earliest_end_time:  # Choose earliest end time
                    earliest_end_time = end_time
                    best_operation = (job_id, op_idx)
                    best_machine = (machine, processing_time, start_time) #Include start time
