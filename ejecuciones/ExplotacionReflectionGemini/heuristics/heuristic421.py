...
def heuristic(input_data):
    """Greedy makespan minimization with local search to balance machine load."""
        ...
        if len(schedule[job_id])>1:
            for i in range(len(schedule[job_id])-1):
                for j in range(i+1, len(schedule[job_id])):
                    ...
                    if new_objectives['Balance'] < original_obj['Balance'] and new_objectives['Makespan'] <= original_obj['Makespan']:
