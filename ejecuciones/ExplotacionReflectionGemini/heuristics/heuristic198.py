...
def heuristic(input_data):
    """Combines greedy makespan minimization with local search for balance."""
    ...
    # Local search: improve balance by reassigning machines
    for job_id in range(1, n_jobs + 1):
        for op_idx in range(len(schedule[job_id])):
            ...
            for machine_idx, new_machine in enumerate(machines):
                if new_machine == original_machine:
                    continue

                processing_time = times[machine_idx]
                #Temporarily assign the new machine
                schedule[job_id][op_idx]['Assigned Machine'] = new_machine
                schedule[job_id][op_idx]['Processing Time'] = processing_time

                #Recalculate start times for the current job
                machine_available_time = {m: 0 for m in range(n_machines)}
                job_completion_time = {j: 0 for j in range(1, n_jobs+1)}

                current_job_schedule = []

                for op_idx_re in range(len(schedule[job_id])):
                    machine = schedule[job_id][op_idx_re]['Assigned Machine']
                    processing_time = schedule[job_id][op_idx_re]['Processing Time']
                    start_time = max(machine_available_time[machine], job_completion_time[job_id])
                    end_time = start_time + processing_time
                    schedule[job_id][op_idx_re]['Start Time'] = start_time
                    schedule[job_id][op_idx_re]['End Time'] = end_time
                    machine_available_time[machine] = end_time
                    job_completion_time[job_id] = end_time
                    current_job_schedule.append(schedule[job_id][op_idx_re])
                ...
                if new_objectives['Balance'] < best_objectives['Balance']:
                  # Update the schedule based on the new machine assignments.
                    ...
                    #Revert back to the original machine
                    ...
                    #Need to recalculate the schedule for the job it comes from
