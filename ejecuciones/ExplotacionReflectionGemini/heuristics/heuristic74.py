...
def heuristic(input_data):
    """Combines SPT job selection with machine load balancing for FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        # SPT Job selection within available jobs
        eligible_jobs = list(set([job_id for job_id, _, _ in eligible_operations]))
        if not eligible_jobs:
          break
        job = min(eligible_jobs, key=lambda j: remaining_operations[j])

        #Select operation of that job with smallest makespan
        best_operation = None
