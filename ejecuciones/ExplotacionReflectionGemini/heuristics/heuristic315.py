...
def heuristic(input_data):
    """Combines SPT, machine load balancing, and job operation count."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                # Score combines start time, machine load, remaining operations
                score = start_time + machine_load[machine] + (remaining_operations[job_id] * 0.1)  

                if score < min_score:
                    min_score = score
                    best_operation = (job_id, op_idx, machine, start_time, time)

        if best_operation:
            job_id, op_idx, machine, start_time, time = best_operation
            end_time = start_time + time
            op_num = op_idx + 1
