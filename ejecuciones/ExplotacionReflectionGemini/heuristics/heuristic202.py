...
def heuristic(input_data):
    """Schedules jobs by considering SPT and machine load, prioritizing earliest start time."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            best_time = float('inf')
            best_load = float('inf') # Use to break ties in start time
            ...
            for i in range(len(machines)):
                ...
                elif start_time == best_start_time and machine_load[machine] < best_load:
                    best_machine = machine
                    best_time = end_time
                    best_load = machine_load[machine]
