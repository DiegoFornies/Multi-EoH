...
def heuristic(input_data):
    """FJSSP heuristic: shortest processing time, earliest machine."""
    ...
    while len(scheduled_operations) < sum(len(ops) for ops in jobs.values()):
        ...
        for job_id, op_idx, machines, times in eligible_operations:
            ...
            for m_idx, machine in enumerate(machines):
                ...
                if start_time + processing_time < best_start_time + best_processing_time if best_operation else True:
                    if best_operation is None or start_time + processing_time < best_start_time + best_processing_time :
