...
def heuristic(input_data):
    """Combines greedy makespan and local search for better balance."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                score = remaining + start_time + machine_load[machine]
    ...
    # Phase 2: Balance Improvement (Local Search - Swap Machines) - using Parent 2 local search method
    ...
    #Improvement by swapping machines of same operation to reduce balance
    for job_id in range(1, n_jobs+1):
        for op_idx in range(len(schedule[job_id])):
            ...
            current_operation = schedule[job_id][op_idx]
            original_machine = current_operation['Assigned Machine']
            machines = jobs[job_id][op_idx][0] #Possible Machines

            #Find other machines for the operation
            possible_machines = [m for m in machines if m != original_machine]

            for new_machine in possible_machines:

                temp_schedule = {job_id: [op.copy() for op in schedule[job_id]]} #Local Schedule
                ...
                for op_idx_reschedule in range(len(temp_schedule[job_id])):
                    machine = temp_schedule[job_id][op_idx_reschedule]['Assigned Machine']
                    processing_time = temp_schedule[job_id][op_idx_reschedule]['Processing Time']

                    start_time = max(machine_available_time[machine], job_completion_time[job_id])
                    end_time = start_time + processing_time

                    temp_schedule[job_id][op_idx_reschedule]['Start Time'] = start_time
                    temp_schedule[job_id][op_idx_reschedule]['End Time'] = end_time

                    machine_available_time[machine] = end_time
                    job_completion_time[job_id] = end_time
                    current_job_schedule.append(temp_schedule[job_id][op_idx_reschedule])
