...
def heuristic(input_data):
    """Combines greedy makespan and local search for FJSSP, improving balance."""
    ...
    # Phase 1: Greedy Scheduling with load consideration
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                score = start_time
                # Load balance consideration to avoid bottleneck
                score += machine_available_time[machine]*0.1
    ...
    # Phase 2: Local Search (Reassignment) for balance
    ...
    num_iterations = 5 # Limit to avoid large delays.

    # Local Improvement by reassignment
            ...
            for op_idx in range(len(schedule[job_id])):
                ...
                for new_machine in machines:
                    if new_machine != original_machine:
                        ...
                        # Recalculate start/end times to ensure feasibility
