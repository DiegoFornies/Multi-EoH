...
def heuristic(input_data):
    """Combines SPT, machine load, and job completion time."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                priority = start_time + machine_load[machine] #+ job_last_end_time[job_id]
            ...
            # Overall priority based on start time and priority score
            combined_priority = best_start_time + best_priority
            if combined_priority < min_combined_priority:
                min_combined_priority = combined_priority
                best_operation = (job_id, op_idx, op_data, best_machine, best_start_time, best_time)


        job_id, op_idx, op_data, machine, start_time, end_time = best_operation
        machines, times = op_data
