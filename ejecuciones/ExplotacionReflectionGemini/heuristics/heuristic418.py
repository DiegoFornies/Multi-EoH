...
def heuristic(input_data):
    """Schedules jobs considering SPT, machine load, and remaining operations."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                # Combine start time, remaining operations, machine load, and processing time in score.
                # Prioritize minimizing start time first, then balancing machine load.
                score = start_time + 0.5 * remaining + 0.1 * machine_load_factor + 0.4 * time
