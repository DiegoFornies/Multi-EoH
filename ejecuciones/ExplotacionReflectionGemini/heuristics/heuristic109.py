...
def heuristic(input_data):
    """Schedules jobs using SPT, considering machine load & job dependencies."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                if time < best_processing_time or (time == best_processing_time and end_time < best_end_time):
                    best_end_time = end_time
                    best_start_time = start_time
                    best_machine = machine
                    best_processing_time = time
            
            if best_end_time < min_makespan:
        ...
        job_last_end_time[job_id] = start_time + processing_time
