...
def heuristic(input_data):
    """
    Combines makespan minimization with balance improvement through local search (swap).
    Hybrid approach using greedy init and local search refinement.
    """
    ...
    # Phase 1: Makespan Minimization (Greedy - Earliest Start Time)
            ...
            for i in range(len(schedule[job_id])-1):
                for j in range(i+1, len(schedule[job_id])):
                    ...
                    if new_objectives['Balance'] < original_obj['Balance']:
                        ...
                        best_schedule = {job_id_: [op.copy() for op in schedule[job_id_]] for job_id_ in schedule}
