...
def heuristic(input_data):
    ...
    while available_operations:
        ...
        for job_id, op_idx in available_operations:
            ...
            #Prioritize machines with earlier available times, breaking ties by shorter processing time
            candidate_machines = []
            for machine_idx, machine in enumerate(machines):
                processing_time = times[machine_idx]
                start_time = max(machine_available_time[machine], job_completion_time[job_id])
                candidate_machines.append((machine, processing_time, start_time))
                
            candidate_machines.sort(key = lambda x: (x[2], x[1])) #Sort by start time, then processing time

            for machine, processing_time, start_time in candidate_machines:
