...
def heuristic(input_data):
    """Schedules jobs using a priority rule: fewest remaining operations."""
    ...
    remaining_operations = {job: len(jobs[j]) for j in jobs}
    ...
    while len(scheduled_operations) < sum(len(ops) for ops in jobs.values()):
        ...
        for job_id, op_idx, op_data in operations:
            if (job_id, op_idx) in scheduled_operations:
                continue
            if all((job_id, prev_op_idx) in scheduled_operations for prev_op_idx in range(op_idx)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for m_idx, machine in enumerate(machines):
                ...
                if remaining < min_remaining or (remaining == min_remaining and start_time < best_start_time):
