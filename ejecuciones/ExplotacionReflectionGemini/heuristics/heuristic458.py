...
def heuristic(input_data):
    """Hybrid heuristic: Combines idle time, remaining operations, and machine load."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                score = remaining + start_time + (machine_load[machine] / (sum(machine_load.values()) + 1e-6))  # Normalize machine_load
                if score < min_score:
                    min_score = score
