...
def heuristic(input_data):
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        if best_operation is None and eligible_operations:
          # If all end times are infinite, choose an eligible operation
          ...
          for job_id, op_idx, op_data in eligible_operations:
            ...
            for job_id, op_idx, op_data in eligible_operations:
              ...
              for i in range(len(machines)):
                  ...
                  end_time = start_time + time
                  if end_time == min_end_time:
                      best_operation = (job_id, op_idx, op_data, machine, start_time)
                      break
