...
def heuristic(input_data):
    """Combines SPT, machine load balancing, and job order for FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            best_time = float('inf')
            best_load = float('inf')
            shortest_processing_time = float('inf') # SPT addition
            ...
            for i in range(len(machines)):
                ...
                if start_time < best_start_time:
                    ...
                    if machine_load[machine] < best_load:
                        ...
                        best_load = machine_load[machine]
                        shortest_processing_time = time
                    elif machine_load[machine] == best_load and time < shortest_processing_time:
                        best_machine = machine
                        best_time = end_time
                        shortest_processing_time = time
        ...
        if best_operation is not None: #Handle the case when no operation is feasible
            ...
            #If no operation could be scheduled, break to avoid infinite loop.
            break
