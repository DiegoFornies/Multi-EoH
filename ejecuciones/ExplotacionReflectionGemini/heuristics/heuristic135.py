...
def heuristic(input_data):
    """Combines shortest processing time and machine load balancing for FJSSP."""
    ...
    while len(scheduled_operations) < sum(len(ops) for ops in jobs.values()):
        ...
        for job_id, op_idx, machines, times in eligible_operations:
            ...
            for m_idx, machine in enumerate(machines):
                ...
                if end_time < best_start_time + best_processing_time:
                    best_start_time = start_time
                    best_machine = machine
                    best_processing_time = processing_time
                    best_operation = (job_id, op_idx, machines, times)
                elif end_time == best_start_time + best_processing_time and machine_load[machine] < machine_load[best_machine]:
                        best_start_time = start_time
                        best_machine = machine
                        best_processing_time = processing_time
                        best_operation = (job_id, op_idx, machines, times)
