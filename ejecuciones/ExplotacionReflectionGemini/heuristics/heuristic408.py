...
def heuristic(input_data):
    """Greedy makespan minimization + local search (swap and reassign) to improve balance."""
    ...
    #Combined Improvement (Swap and Reassign)
    for job_id in range(1, n_jobs + 1):
        if len(schedule[job_id]) > 1:
            for i in range(len(schedule[job_id])):
                
                #Reassignment
                original_machine = schedule[job_id][i]['Assigned Machine']
                machines = jobs[job_id][i][0]
                ...
                for new_machine in machines:
                    if new_machine != original_machine:
                        ...
                        temp_schedule[job_id][i]['Assigned Machine'] = new_machine
                        
                        processing_time = jobs[job_id][i][1][machines.index(new_machine)]
                        temp_schedule[job_id][i]['Processing Time'] = processing_time
                        ...
                        for job_id_re in range(1, n_jobs + 1):
                            for op_idx_re in range(len(jobs[job_id_re])):
                                machines_available = jobs[job_id_re][op_idx_re][0]
                                if job_id_re == job_id and op_idx_re == i:
                                    machine = new_machine
                                else:
                        ...
                        if is_feasible:
                            ...
                            if new_objectives['Balance'] < best_objectives['Balance']:
                                ...
                                for k, v in temp_schedule.items():
                                    best_schedule[k] = [op.copy() for op in v]
        ...
        if len(schedule[job_id]) > 1:
            for i in range(len(schedule[job_id])-1):
                for j in range(i+1, len(schedule[job_id])):
                    original_schedule = {}
                    for k, v in schedule.items():
                        original_schedule[k] = [op.copy() for op in v]
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        for op_idx_re in range(len(jobs[job_id_re])):
                            machines_available = jobs[job_id_re][op_idx_re][0]
                            if job_id_re == job_id and (op_idx_re == i or op_idx_re == j):
                                machine = temp_schedule[job_id_re][op_idx_re]['Assigned Machine']
                            else:
                                machine = temp_schedule[job_id_re][op_idx_re]['Assigned Machine']
