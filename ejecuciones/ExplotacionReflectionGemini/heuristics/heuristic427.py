...
def heuristic(input_data):
    """Combines makespan minimization & balance improvement."""
    ...
    # Phase 1: Makespan Minimization (Greedy + SPT)
    ...
    while available_operations:
        ...
        shortest_processing_time = float('inf') # SPT
        ...
        for job_id, op_idx in available_operations:
            ...
            for machine_idx, machine in enumerate(machines):
                ...
                if start_time < earliest_start_time:
                    ...
                    shortest_processing_time = processing_time # Initialize SPT
    ...
    #Improvement by reassigning operations to balance load
    for job_id in range(1,n_jobs+1):
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine == original_machine:
                    continue #Skip the original machine
                ...
                for op_idx_inner in range(len(temp_schedule[job_id])):
                    machine = temp_schedule[job_id][op_idx_inner]['Assigned Machine']
                    processing_time = temp_schedule[job_id][op_idx_inner]['Processing Time']
                    
                    start_time = max(machine_available_time[machine], job_completion_time[job_id])
                    end_time = start_time + processing_time
                    
                    temp_schedule[job_id][op_idx_inner]['Start Time'] = start_time
                    temp_schedule[job_id][op_idx_inner]['End Time'] = end_time
                    
                    machine_available_time[machine] = end_time
                    job_completion_time[job_id] = end_time
                    current_job_schedule.append(temp_schedule[job_id][op_idx_inner])
