...
def heuristic(input_data):
    """Combines EFT greedy and local search (machine reassignment) to balance load."""
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    # Deep copy is essential to avoid modifying original schedule
                    ...
                    # Recalculate schedule based on the reassignment
                    for job_id_re in range(1, n_jobs + 1):
                        new_schedule[job_id_re] = []
                        op_completion_time = 0  # Completion time of the previous operation in the job.
                    ...
                    # Consider both makespan and balance
                    if new_objectives['Balance'] < best_objectives['Balance'] and new_objectives['Makespan'] <= best_objectives['Makespan'] :
                        ...
                        schedule = new_schedule #update schedule
