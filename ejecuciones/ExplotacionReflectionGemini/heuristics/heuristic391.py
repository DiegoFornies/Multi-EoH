...
def heuristic(input_data):
    ...
    machine_load = {m: 0 for m in range(n_machines)} # Add machine load
    
    # Initialize schedule
    for job_id in range(1, n_jobs + 1):
        schedule[job_id] = []

    # Phase 1: Makespan Minimization (Greedy with SPT and Machine Load)
    ...
    while available_operations:
        ...
        for job_id, op_idx in available_operations:
            ...
            for machine_idx, machine in enumerate(machines):
                ...
                # Consider machine load for tie-breaking
                if end_time < earliest_end_time:
                    earliest_end_time = end_time
                    best_operation = (job_id, op_idx)
                    best_machine = (machine, processing_time)
                elif end_time == earliest_end_time and machine_load[machine] < machine_load.get(best_machine[0], float('inf')):  # Use get to avoid KeyError
                    best_operation = (job_id, op_idx)
                    best_machine = (machine, processing_time)
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    if new_objectives['Balance'] < best_objectives['Balance']:
                        ...
                        machine_load = temp_machine_load
