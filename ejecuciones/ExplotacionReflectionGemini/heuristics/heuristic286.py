...
def heuristic(input_data):
    """Combines makespan minimization with balance improvement via local search."""
    ...
    # Phase 1: Makespan Minimization (Greedy) considering idle time
    ...
    # Improvement by swapping operations of the same job to reduce balance
    for job_id in range(1, n_jobs + 1):
            ...
            for i in range(len(schedule[job_id]) - 1):
                for j in range(i + 1, len(schedule[job_id])):
                    ...
                    try:
                        ...
                        continue  # Handle cases where machine is not available for operation
