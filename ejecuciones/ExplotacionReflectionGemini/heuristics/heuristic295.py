...
def heuristic(input_data):
    """Greedy makespan init, local search (swap/reassign) to balance machine load."""
    ...
    # Combined swap and reassignment with best improvement strategy
            ...
            if len(schedule[job_id]) > 1:
                for i in range(len(schedule[job_id]) - 1):
                    for j in range(i + 1, len(schedule[job_id])):
                        original_schedule = {k: [op.copy() for op in v] for k, v in schedule.items()} #Deep copy
                        ...
                        if new_objectives['Balance'] < best_objectives['Balance']:
                            best_schedule = {k: [op.copy() for op in v] for k, v in new_schedule.items()}  # Deep copy
                ...
                for new_machine in machines:
                    if new_machine != original_machine:
                        ...
                        if new_objectives['Balance'] < best_objectives['Balance']:
                            ...
                            pass # Reassignment does not result in better balance
