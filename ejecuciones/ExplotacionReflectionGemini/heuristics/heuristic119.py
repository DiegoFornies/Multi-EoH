...
def heuristic(input_data):
    """FJSSP heuristic: min makespan, then balance/separation."""
    ...
    machine_load = {m: 0 for m in range(n_machines)} #Keep track machine load
    ...
    while available_operations:
        ...
        for job_id, op_idx in available_operations:
            ...
            for machine_idx, machine in enumerate(machines):
                ...
                #Prioritize minimizing makespan, then balance machine load.
                if end_time < earliest_end_time:
                    earliest_end_time = end_time
                    best_operation = (job_id, op_idx)
                    best_machine = (machine, processing_time)
                elif end_time == earliest_end_time: #If same makespan, balance load
                    if machine_load[machine] < machine_load.get(best_machine[0], float('inf')):
                        best_operation = (job_id, op_idx)
                        best_machine = (machine, processing_time)
