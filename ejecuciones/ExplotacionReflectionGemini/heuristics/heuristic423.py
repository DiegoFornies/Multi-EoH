...
def heuristic(input_data):
    """Combines greedy and local search, prioritizing balance."""
    ...
    # Local search with reassignment to balance machine load
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    new_schedule = {}
                    is_feasible = True # Ensure feasibility
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            if len(new_schedule[job_id_re]) > 0:

                                if start_time < new_schedule[job_id_re][-1]['End Time']:
                                    is_feasible = False
                                    break
