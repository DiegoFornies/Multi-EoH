...
def heuristic(input_data):
    """Combines greedy makespan with machine load balancing and local search for FJSSP."""
    ...
    # Local Search with combined makespan and balance and feasibility check
                ...
                if new_machine != original_machine:
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            processing_times_re = jobs[job_id_re][op_idx_re][1]

                            if job_id_re == job_id and op_idx_re == op_idx:
                                machine = new_machine
                                processing_time_re = processing_time
                            else:
                                machine = temp_schedule[job_id_re][op_idx_re]['Assigned Machine'] if job_id_re in temp_schedule and op_idx_re < len(temp_schedule[job_id_re]) else schedule[job_id_re][op_idx_re]['Assigned Machine'] if job_id_re in schedule and op_idx_re < len(schedule[job_id_re]) else machines_re[0]

                                if machine not in machines_re:
                                    is_feasible = False
                                    break
                                processing_time_re = processing_times_re[machines_re.index(machine)]
