...
def heuristic(input_data):
    """FJSSP heuristic: Combines shortest processing time and machine availability."""
    ...
    while len(scheduled_operations) < sum(len(ops) for ops in jobs.values()):
        ...
        for job_id, op_idx, machines, times in eligible_operations:
            ...
            for m_idx, machine in enumerate(machines):
                ...
                if start_time + processing_time < best_start_time + (best_processing_time if best_processing_time is not None else float('inf')):
