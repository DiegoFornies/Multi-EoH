...
def heuristic(input_data):
    ...
    #Improvement by trying all possible machine assignments
    for job_id in range(1,n_jobs+1):
        original_schedule = {job_id: [op.copy() for op in schedule[job_id]]}
        original_obj = objective_functions(schedule)
        
        # Iterate over possible machine assignments for each operation
        for op_idx in range(len(schedule[job_id])):
            ...
            for machine in machines:
                ...
                processing_time = jobs[job_id][op_idx][1][jobs[job_id][op_idx][0].index(machine)]
