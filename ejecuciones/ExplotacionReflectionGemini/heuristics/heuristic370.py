...
def heuristic(input_data):
    """Combines greedy makespan with SPT and local search to balance machine load and reduce makespan."""
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    new_schedule = {}
                    is_feasible = True # Flag indicating feasibility during rescheduling

                    for job_id_re in range(1, n_jobs + 1):
                        new_schedule[job_id_re] = []
                        current_job_ops = jobs[job_id_re] # Access directly
                        for op_idx_re in range(len(current_job_ops)):
                    ...
                    #Secondary Prioritization, if Balance remains equal
