...
def heuristic(input_data):
    """Combines idle time, SPT, machine load, and remaining operations."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                # Score function: prioritize low start time, balanced load, few remaining operations
                score = start_time + 0.5 * remaining + 0.2 * machine_load_factor - 0.1 * idle_time + 0.3 * time
