...
def heuristic(input_data):
    ...
    while any(remaining_operations := {j: len(jobs[j]) - len(schedule[j]) for j in range(1, n_jobs + 1)}.values()):
        eligible_operations = []
        for job_id, op_idx, op_data in operations:
            if len(jobs[job_id]) > len(schedule[job_id]) and op_idx == len(schedule[job_id]):
    ...
    def objective_functions(current_schedule):
        ...
        makespan = max([op['End Time'] for job in current_schedule for op in current_schedule[job]] or [0])
