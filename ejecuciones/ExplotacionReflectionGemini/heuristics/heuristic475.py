...
def heuristic(input_data):
    """Combines greedy scheduling with local search to improve makespan and balance."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                score = start_time + machine_load[machine]  # consider machine load
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                      ...
                      for op_idx_re in range(len(jobs[job_id_re])):
                        ...
                        if job_id_re == job_id and op_idx_re == op_idx:
                          ...
                          orig_machine = None
                          orig_processing_time = None

                          for op in schedule[job_id_re]:
                            if op['Operation'] == op_idx_re + 1:
                              orig_machine = op['Assigned Machine']
                              orig_processing_time = op['Processing Time']
                              break
                          if orig_machine is None or orig_processing_time is None:
                            
                            machine = jobs[job_id_re][op_idx_re][0][0] #default to first machine for that operation
                            processing_time = jobs[job_id_re][op_idx_re][1][0]
                          else:
                      ...
                      new_schedule[job_id_re] = current_job_schedule
