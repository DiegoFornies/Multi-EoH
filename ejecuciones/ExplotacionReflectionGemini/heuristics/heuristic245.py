...
def heuristic(input_data):
    """Hybrid heuristic: Greedy makespan, local search (swap/reassign) to balance."""
    ...
    #Combined swap and reassignment
            ...
            for i in range(len(schedule[job_id])-1):
                for j in range(i+1, len(schedule[job_id])):
                    ...
                    if new_objectives['Balance'] < best_objectives['Balance']:
                        best_schedule = {k: [op.copy() for op in v] for k, v in schedule.items()} #Deep copy
