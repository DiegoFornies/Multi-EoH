...
def heuristic(input_data):
    """Combines idle time, machine load, and remaining operations for FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                score = remaining + start_time + 0.1 * machine_load[machine] #minimize idle time, remaining operation and balance the load
                if score < min_score:
                    min_score = score
