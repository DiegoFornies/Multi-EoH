...
def heuristic(input_data):
    """Combines SPT, machine load, and job precedence for FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                # Primary: SPT (Shortest Processing Time)
                # Secondary: Machine Load Balance
            ...
            if best_time < float('inf'): #check if feasible machine is available.
                if best_start_time < float('inf'):
                    best_operation = (job_id, op_idx, op_data, best_machine, best_start_time)

        if best_operation is not None: #check if best operation found.
            ...
            min_next_available = float('inf')
            next_machine = None
            for m in range(n_machines):
                if machine_available_time[m] < min_next_available:
                    min_next_available = machine_available_time[m]
                    next_machine = m
            if next_machine is not None:
                machine_available_time[next_machine] = min_next_available + 1
