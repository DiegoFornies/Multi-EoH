...
def heuristic(input_data):
    """Combines SPT and machine load to schedule jobs, prioritizing earlier start times and balancing machine load."""
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                if start_time < best_start_time:
                    ...
                    # Tie-breaker: Prefer machine with lower load, then shorter processing time
                    if machine_load[machine] < machine_load[best_machine]:
                        best_machine = machine
                        best_time = end_time
                    elif machine_load[machine] == machine_load[best_machine] and time < times[machines.index(best_machine)]:
                        best_machine = machine
                        best_time = end_time
