...
def heuristic(input_data):
    """Hybrid: Greedy makespan minimization + local search for balance."""
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    for k in range(1, n_jobs + 1):
                        temp_schedule[k] = [op.copy() for op in schedule[k]]
                    ...
                    # Recalculate the schedule for feasibility.
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        current_op_idx = 0
                        while current_op_idx < len(temp_schedule[job_id_re]):
                            if job_id_re == job_id and current_op_idx == op_idx:
                                machine = new_machine
                                processing_time = jobs[job_id_re][current_op_idx][1][machines.index(new_machine)]

                            else:
                                machine = temp_schedule[job_id_re][current_op_idx]['Assigned Machine']
                                processing_time = temp_schedule[job_id_re][current_op_idx]['Processing Time']

                            start_time = max(temp_machine_available_time[machine], temp_job_completion_time[job_id_re])
                            end_time = start_time + processing_time

                            temp_schedule[job_id_re][current_op_idx]['Start Time'] = start_time
                            temp_schedule[job_id_re][current_op_idx]['End Time'] = end_time
