...
def heuristic(input_data):
    """Greedy initialization + balance-focused local search."""
        ...
        if len(schedule[job_id])>1:
            for i in range(len(schedule[job_id])):
                for j in range(n_machines):
                    ...
                    original_machine = temp_schedule[job_id][i]['Assigned Machine']
                    
                    machines = jobs[job_id][i][0]
                    if j not in machines:
                        continue
                    
                    processing_time = jobs[job_id][i][1][machines.index(j)]
                    
                    temp_schedule[job_id][i]['Assigned Machine'] = j
