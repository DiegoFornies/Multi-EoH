...
def heuristic(input_data):
    """Schedules jobs with SPT and machine load balancing."""
    ...
    job_priority = {job_id: len(jobs_data[job_id]) for job_id in range(1, n_jobs + 1)}
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        best_operation = None
        min_end_time = float('inf')  # Changed criteria to minimize end time.
