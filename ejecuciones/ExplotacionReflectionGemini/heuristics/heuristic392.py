...
def heuristic(input_data):
    """Greedy makespan with SPT and local search (machine reassignment) to balance load."""
        ...
        for op_idx in range(len(schedule[job_id])):            
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    # Recalculate start and end times based on reassignment
                    ...
                    if (new_objectives['Balance'] < best_objectives['Balance']) or \
                       (new_objectives['Balance'] == best_objectives['Balance'] and new_objectives['Makespan'] < best_objectives['Makespan']):
