...
def heuristic(input_data):
    """Hybrid heuristic: Makespan minimization + balance improvement with machine swapping."""
    ...
    # Phase 2: Balance Improvement (Local Search - Single Machine Swap)
    ...
    #Iterate over the schedule and try to assign a different machine to an operation
    for job_id in range(1, n_jobs + 1):
        for op_idx in range(len(schedule[job_id])):
            ...
            for i, new_machine in enumerate(machines):
                if new_machine != original_machine:
                    ...
                    #Update start and end times for the whole job after the change
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        for op_idx_re in range(len(schedule[job_id_re])):
                            ...
                            start_time = max(machine_available_time_temp[assigned_machine], job_completion_time_temp[job_id_re])
                            end_time = start_time + proc_time

                            temp_schedule[job_id_re][op_idx_re]['Start Time'] = start_time
                            temp_schedule[job_id_re][op_idx_re]['End Time'] = end_time

                            machine_available_time_temp[assigned_machine] = end_time
                    ...
                    if new_objectives['Balance'] < best_objectives['Balance']:
                        best_schedule = {k: [op.copy() for op in v] for k, v in temp_schedule.items()}
