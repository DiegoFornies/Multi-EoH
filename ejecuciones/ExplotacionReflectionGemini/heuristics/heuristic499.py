...
def heuristic(input_data):
    """FJSSP heuristic with makespan minimization and load balancing."""
    ...
    while available_operations:
        ...
        for job_id, op_idx in available_operations:
            ...
            for machine_idx, machine in enumerate(machines):
                ...
                # Machine load balancing criteria
                load_score = machine_load[machine] + processing_time

                # Choose machine with earliest end time AND balances load
                if end_time < best_local_time:
                    best_local_time = end_time
                    best_local_machine = (machine, processing_time, load_score, start_time)
                    
            if best_local_machine:
                machine, processing_time, load_score, start_time = best_local_machine
                end_time = start_time + processing_time
