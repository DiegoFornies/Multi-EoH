...
def heuristic(input_data):
    """Combines SPT, machine load, idle time, and balance improvement for FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                # Combine start time, machine load, and idle time.
                idle_time = start_time - machine_available_time[machine] if start_time > machine_available_time[machine] else 0
                combined_score = start_time + 0.1 * machine_load[machine] - 0.05 * idle_time  # Adjust weights as needed
                ...
                elif combined_score == best_combined_score and start_time < best_start_time:
                    best_start_time = start_time
                    best_time = time
                    best_machine = machine


            if best_combined_score < min_combined_score:
                min_combined_score = best_combined_score
                best_operation = (job_id, op_idx, op_data, best_machine, best_start_time, best_time)
                    ...
                    if new_objectives['Balance'] < original_obj['Balance']:
                        ...
                        for job_id_re in range(1, n_jobs+1):
                            for op_idx_re in range(len(schedule[job_id_re])):
                                ...
                                start_time = max(machine_available_time[machine], job_last_end_time[job_id_re])
                                end_time = start_time + processing_time
                                ...
                                machine_available_time[machine] = end_time
                                job_last_end_time[job_id_re] = end_time
