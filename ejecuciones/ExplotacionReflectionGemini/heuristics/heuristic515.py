...
def heuristic(input_data):
    """Combines greedy and local search to minimize makespan and balance."""
    ...
    # Local search with more comprehensive rescheduling
    for job_id in range(1, n_jobs + 1):
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    # Create a temporary schedule based on the initial schedule
                    for j in range(1, n_jobs + 1):
                        temp_schedule[j] = [op.copy() for op in schedule[j]]

                    # Assign the new machine and processing time
                    ...
                    # Reschedule all operations after the changed one to ensure feasibility
                    reschedule_needed = True
                    while reschedule_needed:
                      ...
                      for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            # Update the start and end times in the temp schedule
                            if temp_schedule[job_id_re][op_idx_re]['Start Time'] != start_time or temp_schedule[job_id_re][op_idx_re]['End Time'] != end_time:
