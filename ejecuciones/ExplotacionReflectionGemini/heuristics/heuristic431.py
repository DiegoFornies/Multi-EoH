...
def heuristic(input_data):
    """Combines greedy scheduling, idle time reduction, and load balancing."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                score = remaining + start_time  # Minimize idle time & remaining tasks

                # Load balancing component: prefer machines with lower current load
                machine_load = sum([op['Processing Time'] for j in schedule for op in schedule[j] if op['Assigned Machine'] == machine])
                score += machine_load / 100.0  # Weight the load balancing factor
        ...
        if best_operation:
            ...
            break # If no best operation is found, exit.
