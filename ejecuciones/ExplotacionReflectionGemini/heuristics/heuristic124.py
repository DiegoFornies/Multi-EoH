    ...
    while available_operations:
        ...
        for job_id, op_num in available_operations:
            ...
            for m_idx, m in enumerate(machines):
                start_time = max(machine_available_times[m], job_last_end_time[job_id]) if op_num > 1 else machine_available_times[m]
                end_time = start_time + times[m_idx]

                if end_time < best_start_time:
                    best_start_time = end_time
                    chosen_machine = m
                    chosen_time = times[m_idx]

            if best_start_time < min_end_time:
                min_end_time = best_start_time
                best_op = (job_id, op_num, chosen_machine, chosen_time, start_time)
