...
def heuristic(input_data):
    """Combines SPT, machine load, and remaining operations, prioritizing machine idle time."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                #Prioritize machine idle time as parent 2, also consider parent 1's features

                score = idle_time

                if score < min_idle_time:
                    min_idle_time = score
                    best_operation = (job_id, op_idx, op_data)
                    best_machine = (machine, time, start_time)


        if best_operation:
            job_id, op_idx, op_data = best_operation
            machine, time, start_time = best_machine
            end_time = start_time + time
            op_num = op_idx + 1
