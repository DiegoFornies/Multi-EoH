...
def heuristic(input_data):
    """Hybrid heuristic for FJSSP combining greedy makespan and local search."""
    ...
    # Improvement by reassignment and insertion (Local Search)
    for job_id in range(1,n_jobs+1):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        temp_schedule[job_id_re] = [op.copy() for op in schedule[job_id_re]]
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            processing_time = jobs[job_id_re][op_idx_re][1][jobs[job_id_re][op_idx_re][0].index(machine)] if job_id_re == job_id and op_idx_re == op_idx else temp_schedule[job_id_re][op_idx_re]['Processing Time'] #jobs[job_id_re][op_idx_re][1][temp_schedule[job_id_re][op_idx_re]['Assigned Machine']]
