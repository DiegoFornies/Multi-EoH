...
def heuristic(input_data):
    """Combines greedy scheduling, idle time minimization, and local search for machine load balancing."""
                ...
                if new_machine != original_machine:
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            found = False
                            for k, ops in temp_schedule.items():
                                for op in ops:
                                    if op['Operation'] == op_idx_re + 1 and k == job_id_re:
                                        assigned_machine = op['Assigned Machine']
                                        proc_time = op['Processing Time']
                                        found = True
                                        break
                                if found:
                                    break
