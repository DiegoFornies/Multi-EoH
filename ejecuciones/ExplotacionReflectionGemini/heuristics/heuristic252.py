...
def heuristic(input_data):
    """Combines machine load, SPT, and remaining ops to schedule FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                # Minimize start time, balance machine load, consider processing time, and remaining ops
                score = start_time + 0.5 * machine_load_factor + time + remaining #minimize start, remining op, balance machine, time
