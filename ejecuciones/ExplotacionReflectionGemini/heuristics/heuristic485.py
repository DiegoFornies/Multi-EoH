...
def heuristic(input_data):
    """Combines SPT, machine load, and earliest machine availability."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                score = start_time + machine_load_factor + time #minimize start, balance machine, time
                #find the earliest machine available for this operation
                if score < min_score:
                    min_score = score
