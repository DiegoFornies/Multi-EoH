...
def heuristic(input_data):
    """Combines greedy makespan, reassignment, and swap to balance load."""
    ...
    # Phase 2: Balance Improvement (Local Search)
    ...
    def calculate_makespan(current_schedule):
        makespan = 0
        for job_id in current_schedule:
            if current_schedule[job_id]:  # Corrected to use current_schedule
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    # Recalculate entire schedule for feasibility.
            ...
            #Swap operations of same job
            if len(schedule[job_id])>1:
                for i in range(len(schedule[job_id])-1):
                    for j in range(i+1, len(schedule[job_id])):

                        original_schedule_swap = {job_id: [op.copy() for op in schedule[job_id]]}
                        original_obj_swap = objective_functions(schedule)
                        temp_schedule_swap = {job_id: [op.copy() for op in schedule[job_id]]}
                        
                        op1_original_machine = temp_schedule_swap[job_id][i]['Assigned Machine']
                        op2_original_machine = temp_schedule_swap[job_id][j]['Assigned Machine']
                        
                        temp_schedule_swap[job_id][i]['Assigned Machine'] = op2_original_machine
                        temp_schedule_swap[job_id][j]['Assigned Machine'] = op1_original_machine
                        
                        machines1 = jobs[job_id][i][0]
                        machines2 = jobs[job_id][j][0]
                        
                        if temp_schedule_swap[job_id][i]['Assigned Machine'] not in machines1 or \
                            temp_schedule_swap[job_id][j]['Assigned Machine'] not in machines2:
                            continue
                        
                        processing_time_op1 = jobs[job_id][i][1][machines1.index(temp_schedule_swap[job_id][i]['Assigned Machine'])]
                        processing_time_op2 = jobs[job_id][j][1][machines2.index(temp_schedule_swap[job_id][j]['Assigned Machine'])]

                        temp_schedule_swap[job_id][i]['Processing Time'] = processing_time_op1
                        temp_schedule_swap[job_id][j]['Processing Time'] = processing_time_op2
                        ...
                        for op_idx_swap in range(len(temp_schedule_swap[job_id])):
                            machine = temp_schedule_swap[job_id][op_idx_swap]['Assigned Machine']
                            processing_time = temp_schedule_swap[job_id][op_idx_swap]['Processing Time']
                            
                            start_time = max(temp_machine_available_time[machine], temp_job_completion_time[job_id])
                            end_time = start_time + processing_time
                            
                            temp_schedule_swap[job_id][op_idx_swap]['Start Time'] = start_time
                            temp_schedule_swap[job_id][op_idx_swap]['End Time'] = end_time
                            
                            temp_machine_available_time[machine] = end_time
                            temp_job_completion_time[job_id] = end_time
                            current_job_schedule.append(temp_schedule_swap[job_id][op_idx_swap])
                            
                        schedule[job_id] = current_job_schedule
                        new_objectives_swap = objective_functions(schedule)
                        
                        if new_objectives_swap['Balance'] < best_objectives['Balance']:
                            best_schedule = schedule
                            best_objectives = new_objectives_swap
