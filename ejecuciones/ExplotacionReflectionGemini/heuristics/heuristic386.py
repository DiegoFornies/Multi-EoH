...
def heuristic(input_data):
    """A heuristic for FJSSP combining idle time & load balancing."""
    ...
    while any(len(schedule[job]) < len(jobs[job]) for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                # Combines idle time (start_time) & normalized machine load
                normalized_load = machine_load[machine] / (sum(machine_load.values()) + 1e-9) if sum(machine_load.values()) > 0 else 0
                score = start_time + normalized_load * 10
