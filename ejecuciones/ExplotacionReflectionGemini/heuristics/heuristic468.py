...
def heuristic(input_data):
    """Hybrid heuristic: SPT+Machine Load+Local Search for FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                score = start_time + remaining + machine_load_factor + time # minimize start, remining op, balance machine, time
    ...
    # Local Search - Machine Reassignment (from parent 2)
    ...
    def objective_functions(current_schedule):
        ...
        n_machines_used = len([m for m in machine_load in machine_load if machine_load[m] > 0])
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    temp_schedule = {job_id_temp: [op.copy() for op in schedule[job_id_temp]] for job_id_temp in schedule}
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        temp_job_completion_time[job_id_re] = 0
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                       ...
                       for op_idx_re in range(len(jobs_data[job_id_re])):
                           ...
                           temp_schedule[job_id_re][op_idx_re] = {
