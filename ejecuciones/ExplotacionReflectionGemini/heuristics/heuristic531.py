...
def heuristic(input_data):
    """Combines greedy scheduling and local search for FJSSP."""
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    new_schedule = {}
                    feasible = True
                    op_completion_time_job = {j:0 for j in range(1, n_jobs + 1)}
                    ...
                    for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(jobs[job_id_re])):
                            ...
                            start_time = max(temp_machine_available_time[machine], op_completion_time_job[job_id_re])
                            ...
                            op_completion_time_job[job_id_re] = end_time
                        temp_job_completion_time[job_id_re] = op_completion_time_job[job_id_re]

                    new_objectives = objective_functions(new_schedule)

                    if new_objectives['Balance'] < best_objectives['Balance'] and new_objectives['Makespan'] <= best_objectives['Makespan'] and feasible:
