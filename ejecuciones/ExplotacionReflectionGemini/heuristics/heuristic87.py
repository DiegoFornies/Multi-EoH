...
def heuristic(input_data):
    """Combines machine load, SPT, and operation feasibility."""
    ...
    while available_operations:
        ...
        for job_id, op_num in available_operations:
            ...
            for m_idx, m in enumerate(machines):
                start_time = max(machine_available_times[m], job_completion_times[job_id]) if op_num > 1 else machine_available_times[m]
                end_time = start_time + times[m_idx]
                
                if end_time < min_end_time:
                    min_end_time = end_time
                    best_op = (job_id, op_num, m, times[m_idx], start_time)
        ...
        if job_id not in solution:
            solution[job_id] = []
        solution[job_id].append({'Operation': op_num, 'Assigned Machine': assigned_machine, 'Start Time': start_time, 'End Time': end_time, 'Processing Time': processing_time})
