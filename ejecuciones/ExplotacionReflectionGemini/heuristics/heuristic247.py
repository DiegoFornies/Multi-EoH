...
def heuristic(input_data):
    """FJSSP heuristic: Combines idle time, remaining operations, & SPT."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                # Combination of remaining operations, start time (idle time), and processing time (SPT)
                score = remaining + start_time + processing_time # Minimize idle time, remaining ops and processing time.
                if score < min_score:
                    min_score = score
        ...
        if best_operation:
            ...
            break # If no best operation found, break the loop
