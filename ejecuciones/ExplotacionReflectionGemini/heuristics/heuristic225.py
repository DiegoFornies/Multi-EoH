...
def heuristic(input_data):
    """Combines SPT, machine load, and earliest availability for FJSSP."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        best_operation = None
        min_end_time = float('inf')  #Changed from min_start_time to min_end_time

        for job_id, op_idx, op_data in eligible_operations:
            machines, times = op_data

            best_machine = -1
            best_end_time = float('inf') #Changed from start time to end_time
            best_time = float('inf')
            best_load = float('inf')
            ...
            for i in range(len(machines)):
                ...
                # Primary criteria: Minimize end time.
                # Secondary criteria: Balance machine load.
                if end_time < best_end_time:
                    best_end_time = end_time  #Changed from start to end
                    best_start_time = start_time #Added
                    best_time = time
                    best_machine = machine
                    best_load = machine_load[machine]
                elif end_time == best_end_time: #Changed from start to end
                    if machine_load[machine] < best_load:
                        best_end_time = end_time  #Changed from start to end
                        best_start_time = start_time #Added
                        best_machine = machine
                        best_time = time
            ...
            if best_end_time < min_end_time: #Changed from start to end
                min_end_time = best_end_time #Changed from start to end
                best_operation = (job_id, op_idx, op_data, best_machine, best_start_time, best_time) #Pass best_time

        job_id, op_idx, op_data, machine, start_time, time = best_operation
        #machines, times = op_data #No need
        #time = next(t for i, t in enumerate(times) if machines[i] == machine) #No need
        end_time = start_time + time
        op_num = op_idx + 1
