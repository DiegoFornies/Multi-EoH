...
def heuristic(input_data):
    """Combines greedy makespan with local search to balance load."""
    ...
    #Local search improvement (reassignment)
    for job_id in range(1, n_jobs + 1):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    if new_objectives['Balance'] < best_objectives['Balance']:
                        ...
                        schedule = {k: [op.copy() for op in v] for k, v in new_schedule.items()} #update main schedule
