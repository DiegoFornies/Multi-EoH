...
def heuristic(input_data):
    """Combines machine load and job dependencies."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        min_machine_load = float('inf') # NEW : balance machine load
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            #Prioritize operations based on remaining operations, makespan, and machine load
            if remaining < min_remaining:
                min_remaining = remaining
                min_makespan = best_time
                min_machine_load = machine_available_time[best_machine] # NEW
                best_operation = (job_id, op_idx, op_data, best_machine, best_start_time)
            elif remaining == min_remaining:
                if best_time < min_makespan:
                    min_makespan = best_time
                    min_machine_load = machine_available_time[best_machine] # NEW
                    best_operation = (job_id, op_idx, op_data, best_machine, best_start_time)
                elif best_time == min_makespan and machine_available_time[best_machine] < min_machine_load: # Tiebreaker: choose machine with smaller load
                    min_machine_load = machine_available_time[best_machine] # NEW
                    best_operation = (job_id, op_idx, op_data, best_machine, best_start_time)
