...
def heuristic(input_data):
    """Combines greedy makespan and local search (swap and reassignment) for FJSSP."""
    ...
    # Phase 2: Balance Improvement (Local Search - Swap and Reassignment)
    ...
    #Improvement by reassignment & swapping
    for job_id in range(1,n_jobs+1):
        for op_idx in range(len(schedule[job_id])):

            #Reassignment

            original_schedule_reassign = {job_id: [op.copy() for op in schedule[job_id]]}
            original_obj_reassign = objective_functions(schedule)
            ...
            # Swapping only if the job has more than one operation
            if len(schedule[job_id])>1:
                for i in range(len(schedule[job_id])-1):
                    for j in range(i+1, len(schedule[job_id])):
                        ...
                        for op_idx_swap in range(len(temp_schedule[job_id])):
                            machine = temp_schedule[job_id][op_idx_swap]['Assigned Machine']
                            processing_time = temp_schedule[job_id][op_idx_swap]['Processing Time']
                            
                            start_time = max(machine_available_time[machine], job_completion_time[job_id])
                            end_time = start_time + processing_time
                            
                            temp_schedule[job_id][op_idx_swap]['Start Time'] = start_time
                            temp_schedule[job_id][op_idx_swap]['End Time'] = end_time
                            
                            machine_available_time[machine] = end_time
                            job_completion_time[job_id] = end_time
                            current_job_schedule.append(temp_schedule[job_id][op_idx_swap])
