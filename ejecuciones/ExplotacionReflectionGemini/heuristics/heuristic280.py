...
def heuristic(input_data):
    """Adaptively adjusts machine selection based on load balance."""
    ...
    while available_operations:
        ...
        for job_id, op_idx in available_operations:
            ...
            for machine_idx, machine in enumerate(machines):
                candidates.append((machine, times[machine_idx], machine_load[machine]))
            
            # Sort by machine load (ascending) then processing time (ascending)
            candidates.sort(key=lambda x: (x[2], x[1]))
            
            # Choose the machine with least load
            machine, processing_time, _ = candidates[0]
