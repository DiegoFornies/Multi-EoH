...
def heuristic(input_data):
    """FJSSP heuristic: Combines idle time, remaining ops, & machine balance."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                machine_load = sum([op['Processing Time'] for job_num in schedule for op in schedule[job_num] if op['Assigned Machine'] == machine])
                score = remaining + start_time + (machine_load/100) # minimize idle time, jobs with fewer remaining tasks, and balance machine load.
                if score < min_score:
                    min_score = score
