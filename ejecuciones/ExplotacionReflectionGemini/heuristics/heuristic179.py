...
def heuristic(input_data):
    """
    Heuristic for FJSSP minimizing makespan with machine load balancing.
    Uses SPT-like dispatching and load balancing to improve initial solution.
    """
    ...
    while available_operations:
        ...
        for job_id, op_idx in available_operations:
            ...
            # Apply SPT-like dispatching, but consider load balancing
            ...
            for machine_idx, machine in enumerate(machines):
                ...
                if processing_time < shortest_processing_time:
                    ...
                    best_local_machine = (machine, processing_time, start_time)

            # Load balancing: Choose machine that minimizes load increase
            machine, processing_time, start_time = best_local_machine
            end_time = start_time + processing_time
            
            if end_time < earliest_end_time:
                earliest_end_time = end_time
                best_operation = (job_id, op_idx)
                best_machine = (machine, processing_time, start_time)
            

        job_id, op_idx = best_operation
        machine, processing_time, start_time = best_machine
