...
def heuristic(input_data):
    """
    A hybrid heuristic combining greedy makespan minimization
    with local search for machine load balancing.
    """
    ...
    # Reassignment with delta evaluation.
    for job_id in range(1,n_jobs+1):
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    original_load = calculate_machine_load(schedule)
                    new_load = original_load.copy()
                    new_load[original_machine] -= schedule[job_id][op_idx]['Processing Time']
                    processing_time = jobs[job_id][op_idx][1][machines.index(new_machine)]
                    new_load[new_machine] += processing_time

                    total_load = sum(new_load.values())
                    n_machines_used = len([m for m in new_load if new_load[m] > 0])

                    if n_machines_used > 0:
                        original_balance = best_objectives['Balance']
                        new_balance = sum([(new_load[m] - (total_load / n_machines_used)) ** 2 for m in new_load if new_load[m] > 0]) / n_machines_used
                        ...
                        if new_balance < original_balance:
                            ...
                            # Only Update if feasible (Makespan is unchanged)
                            temp_machine_available_time = {m: 0 for m in range(n_machines)}
                            temp_job_completion_time = {j: 0 for j in range(1, n_jobs + 1)}
                            feasible = True
                            for job_id2 in range(1, n_jobs + 1):
                                for op_idx2 in range(len(temp_schedule[job_id2])):
                                    machine = temp_schedule[job_id2][op_idx2]['Assigned Machine']
                                    processing_time = temp_schedule[job_id2][op_idx2]['Processing Time']
                                    start_time = max(temp_machine_available_time[machine], temp_job_completion_time[job_id2])
                                    end_time = start_time + processing_time
                                    temp_schedule[job_id2][op_idx2]['Start Time'] = start_time
                                    temp_schedule[job_id2][op_idx2]['End Time'] = end_time
                                    temp_machine_available_time[machine] = end_time
                                    temp_job_completion_time[job_id2] = end_time
                            new_obj = objective_functions(temp_schedule)

                            if new_obj['Makespan'] <= best_objectives['Makespan']:
                                best_schedule = temp_schedule
                                best_objectives = new_obj
