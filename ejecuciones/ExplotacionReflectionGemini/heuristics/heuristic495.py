...
def heuristic(input_data):
    """
    A hybrid heuristic for FJSSP using a greedy approach with lookahead and local search for machine reassignment.
    Combines makespan minimization with machine load balancing.
    """
    ...
    while available_operations:
        ...
        for job_id, op_idx in available_operations:
            ...
            # Lookahead: Evaluate multiple machines
            best_machine_local = None
            earliest_end_time_local = float('inf')

            for machine_idx, machine in enumerate(machines):
                processing_time = times[machine_idx]
                start_time = max(machine_available_time[machine], job_completion_time[job_id])
                end_time = start_time + processing_time
                score = end_time + machine_load[machine] # considers load

                if score < earliest_end_time_local:
                    earliest_end_time_local = score
                    best_machine_local = (machine, processing_time)

            if best_machine_local is not None:
                machine, processing_time = best_machine_local
                start_time = max(machine_available_time[machine], job_completion_time[job_id])
                end_time = start_time + processing_time
    ...
    # Local Search: Machine Reassignment
    ...
    #Improvement by reassignment based on load balancing
