...
def heuristic(input_data):
    """Combines job prioritization, machine load balancing, and operation dependencies."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i in range(len(machines)):
                ...
                elif end_time == best_time and machine_load[machine] < machine_load[best_machine if best_machine != -1 else 0]:  #Compare machine load
                    best_machine = machine
                    best_start_time = start_time
        ...
        machine_load[machine] += time # Update machine load
