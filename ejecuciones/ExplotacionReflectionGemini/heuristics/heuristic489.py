...
def heuristic(input_data):
    """Combines greedy scheduling with local search for improved FJSSP solutions."""
    ...
    while any(remaining_operations[job] > 0 for job in range(1, n_jobs + 1)):
        ...
        for job_id, op_idx, op_data in eligible_operations:
            ...
            for i, machine in enumerate(machines):
                ...
                # score = start_time + machine_load[machine] + (remaining * 0.1) # Adjusted weight
                score = end_time # Using earliest finish time
        ...
        for op_idx in range(len(schedule[job_id])):
            ...
            for new_machine in machines:
                if new_machine != original_machine:
                    ...
                    new_schedule = {}
                    feasible = True
                    op_completion_times = {j: 0 for j in range(1, n_jobs + 1)}
                    machine_availabilities = {m: 0 for m in range(n_machines)}
                    for job_id_re in range(1, n_jobs + 1):
                        ...
                        for op_idx_re in range(len(schedule[job_id_re])):
                            ...
                            start_time = max(machine_availabilities[machine], op_completion_times[job_id_re])
                            ...
                            machine_availabilities[machine] = end_time
                            op_completion_times[job_id_re] = end_time
