...
def heuristic(input_data):
    """Combines SPT, machine availability, remaining operations, and load balancing for FJSSP."""
    ...
    while available_operations:
        # Prioritize remaining operations, SPT, and machine load. Combines both parent sorting ideas.
        available_operations.sort(key=lambda op: (job_remaining_operations[op['job_id']], min(op['times']), machine_available_time[op['machines'][op['times'].index(min(op['times']))]]))
            ...
            if op['op_idx'] == job_current_operation[op['job_id']]:
                ...
                for job_schedule in schedule.values():
                    for scheduled_op in job_schedule:
                        if scheduled_op['Operation'] == op['op_idx'] and scheduled_op['Assigned Machine'] is not None:
                            is_scheduled = True
                            break
                    if is_scheduled:
                        break
