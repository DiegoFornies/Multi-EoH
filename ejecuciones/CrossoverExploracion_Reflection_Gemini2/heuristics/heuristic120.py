...
def heuristic(input_data):
    """
    A heuristic for FJSSP balancing SPT, machine load, and separation dynamically.
    Prioritizes shorter processing times and earliest available machines and avoids machine overloading.
    """
    ...
    while available_operations:
        ...
        def operation_priority(op):
            min_time = float('inf')
            best_machine = None
            for i in range(len(op['machines'])):
                machine = op['machines'][i]
                time = op['times'][i]
                if time < min_time:
                    min_time = time
                    best_machine = machine
            if best_machine is None:
                return float('inf')  # Return a large value if no suitable machine is found.
            return min_time * load_weights[best_machine]
