...
def heuristic(input_data):
    """
    FJSSP heuristic: SPT, machine load, random tie-breaking for load balancing.
    """
    ...
    while available_operations:
        ...
        #Random choice among best options with consideration to machine load
        min_spt = min([min(op['times']) for op in available_operations])
        best_operations = [op for op in available_operations if min(op['times']) == min_spt]

        #Tie-breaking for balanced machine load
        min_load = min([machine_available_time[op['machines'][op['times'].index(min(op['times']))]] for op in best_operations])
        best_operations = [op for op in best_operations if machine_available_time[op['machines'][op['times'].index(min(op['times']))]] == min_load]

        operation = random.choice(best_operations)
