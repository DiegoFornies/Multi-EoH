...
def heuristic(input_data):
    """
    Combines SPT, machine load, and job dependencies
    with dynamic operation prioritization.
    """
    ...
    while available_operations:
        # Adaptive Priority: SPT + Machine Load (Parent 1) + Handling no available machines
        def operation_priority(op):
            ...
            if not machine_loads:
              return float('inf')
            avg_machine_load = sum(machine_loads) / len(machine_loads)
