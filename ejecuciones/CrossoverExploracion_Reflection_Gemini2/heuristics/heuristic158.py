...
def heuristic(input_data):
    """Schedules jobs using a priority rule based on operation slack time."""
    ...
    while available_operations:
        # Calculate slack time for each operation and sort by it
        def calculate_slack(op):
            min_time = min(op['times'])
            return min_time # Simplified slack calcuation

        available_operations.sort(key=calculate_slack)
            ...
            if op['op_idx'] == job_current_operation[op['job_id']]:
                ...
                for job_schedule in schedule.values():
                    for scheduled_op in job_schedule:
                        if scheduled_op['Operation'] == op['op_idx'] and op['job_id'] == op['job_id']:
                            is_scheduled = True
                            break
                    if is_scheduled:
                        break
