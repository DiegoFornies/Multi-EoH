...
def heuristic(input_data):
    """
    Combines SPT, machine availability, and load balancing
    for efficient FJSSP scheduling.
    """
    ...
    for job_id, job in jobs_data.items():
        for op_idx, (machines, times) in enumerate(job):
            operations.append({
                ...
                'op_idx': op_idx + 1,
                'machines': machines,
                'machines_options': machines,  # Storing available machines and their processing times
                'times_options': times
            })
    ...
    while available_operations:
        ...
        available_operations.sort(key=lambda op: (min(op['times_options']), machine_available_time[op['machines_options'][op['times_options'].index(min(op['times_options']))]]))
