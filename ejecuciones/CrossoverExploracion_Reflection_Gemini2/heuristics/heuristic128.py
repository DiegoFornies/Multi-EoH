...
def heuristic(input_data):
    """
    FJSSP heuristic: SPT with machine load balancing and job dependency.
    """
    ...
    while available_operations:
        # Prioritize SPT and Machine Load, adding job completion time to balance across jobs
        available_operations.sort(key=lambda op: (min(op['times']), machine_available_time[op['machines'][op['times'].index(min(op['times']))]] + job_completion_time[op['job_id']]))
            ...
            if op['op_idx'] == job_current_operation[op['job_id']]:
                ...
                for job_schedule in schedule.values():
                    for scheduled_op in job_schedule:
                        if scheduled_op['Operation'] == op['op_idx'] and scheduled_op['Assigned Machine'] == op['machines'][0] if len(op['machines']) == 1 else scheduled_op['Assigned Machine'] in op['machines']:
                            is_scheduled = True
                            break
                    if is_scheduled:
                        break
