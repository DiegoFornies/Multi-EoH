...
def heuristic(input_data):
    """Combines SPT, machine load, and random selection for FJSSP."""
    ...
    while available_operations:
        ...
        # Prioritize operations: SPT + machine load. Random tie-breaking.
        def operation_priority(op):
            ...
            return min_time + 0.1 * avg_machine_load

        prioritized_operations = sorted(available_operations, key=operation_priority)

        # Random selection from top candidates (within a threshold).
        best_priority = operation_priority(prioritized_operations[0])
        candidates = [op for op in prioritized_operations if operation_priority(op) <= best_priority * 1.1] #adjust threshold
