...
def heuristic(input_data):
    """FJSSP heuristic: Combines LRPT and machine load."""
    ...
    for job_id, job in jobs_data.items():
        ...
        for machines, times in job:
            total_remaining_time += min(times) # use optimistic processing time
        job_remaining_time[job_id] = total_remaining_time
    ...
    while available_operations:
        # Prioritize based on LRPT (Longest Remaining Processing Time), then balance machine load.
        available_operations.sort(key=lambda op: (
            -job_remaining_time[op['job_id']],
            sum(machine_available_time[m] for m in op['machines']) / len(op['machines'])
        ))
        ...
        job_remaining_time[job_id] -= processing_time  # Update the job's remaining processing time
