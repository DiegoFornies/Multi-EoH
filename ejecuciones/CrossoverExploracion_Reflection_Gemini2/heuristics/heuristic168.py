...
def heuristic(input_data):
    ...
    while available_operations:
        # Prioritize SPT and machine load
        available_operations.sort(key=lambda op: (min(op['times']), min([machine_available_time[m] for m in op['machines']])))
            ...
            if op['op_idx'] == job_current_operation[op['job_id']]:
                ...
                for job_schedule in schedule.values():
                    for scheduled_op in job_schedule:
                        scheduled_job_id = op['job_id']
                        if scheduled_op['Operation'] == op['op_idx'] and scheduled_job_id == op['job_id']:
                            is_scheduled = True
                            break
                    if is_scheduled:
                        break
