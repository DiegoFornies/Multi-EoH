...
def heuristic(input_data):
    """
    FJSSP heuristic: Dynamically balances SPT and machine load for makespan & balance.
    Prioritizes separation between consecutive operations of the same job.
    """
    ...
    def calculate_machine_load(machine):
        """Calculates the current load on a machine."""
        load = 0
    ...
    while available_operations:
        # Dynamic prioritization: SPT with machine load balancing.
        available_operations.sort(key=lambda op: (min(op['times']), calculate_machine_load(op['machines'][0]) if len(op['machines']) > 0 else 0))  #Prioritize SPT and balance machine load
        ...
        separation_time = 5  # Add a separation time

        # Find the best machine considering machine load
        for i in range(len(machines)):
            machine = machines[i]
            time = times[i]
            available_time = machine_available_time[machine]
            start_time = max(available_time, job_completion_time[job_id] + separation_time if op_idx > 1 else job_completion_time[job_id]) # Add separation time
        ...
        start_time = max(machine_available_time[best_machine], job_completion_time[job_id] + separation_time if op_idx > 1 else job_completion_time[job_id]) # Add separation time
