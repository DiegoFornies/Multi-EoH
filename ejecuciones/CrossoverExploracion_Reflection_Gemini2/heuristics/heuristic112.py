...
def heuristic(input_data):
    """Combines SPT, EDD, and machine availability for FJSSP."""
    ...
    def calculate_due_date(job_id):
        ...
        remaining_time = 0
        current_op_index = job_current_operation[job_id] - 1
        for i in range(current_op_index, len(jobs_data[job_id])):
            remaining_time += min(jobs_data[job_id][i][1])
        return job_completion_time[job_id] + remaining_time
    ...
    while any(job_current_operation[j] <= len(jobs_data[j]) for j in jobs_data):
        available_operations = [op for op in operations if op['op_idx'] == job_current_operation[op['job_id']]]
        # Prioritize jobs with Earliest Due Date (EDD) then SPT
        available_operations.sort(key=lambda op: (calculate_due_date(op['job_id']), min(op['times'])))
