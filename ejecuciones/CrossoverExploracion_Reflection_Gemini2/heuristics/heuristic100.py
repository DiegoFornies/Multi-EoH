...
def heuristic(input_data):
    """
    FJSSP heuristic minimizing makespan, separation, and balance.
    Dynamically prioritizes SPT based on machine load.
    """
    ...
    while available_operations:
        ...
        for m in range(n_machines):
            ...
            for job_id in jobs_data:
                 for op in schedule.get(job_id, []):
                     if op['Assigned Machine'] == m:
                         load += op['Processing Time']
            machine_load[m] = load
            total_load +=load
        avg_machine_load = total_load / n_machines if n_machines > 0 else 0

        # Prioritize SPT based on machine load
        def operation_key(op):
            min_time = min(op['times'])
            machine_index = op['machines'][op['times'].index(min_time)]
            load_factor = machine_load[machine_index] / avg_machine_load if avg_machine_load > 0 else 1
            return min_time * load_factor

        available_operations.sort(key=operation_key)
            ...
            if op['op_idx'] == job_current_operation[op['job_id']]:
                ...
                for job_schedule in schedule.values():
                    for scheduled_op in job_schedule:
                        if scheduled_op['Operation'] == op['op_idx']:
                            is_scheduled = True
                            break
                    if is_scheduled:
                        break
