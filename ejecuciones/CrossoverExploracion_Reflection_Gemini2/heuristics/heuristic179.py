...
def heuristic(input_data):
    """FJSSP heuristic: Adaptive SPT, load balancing, separation."""
    ...
    while available_operations:
        # Adaptive Weighting
        makespan_urgency = 0.5  # Adjust based on makespan pressure
        remaining_ops_weight = 0.2 # Adjust during runtime based on the problem
        load_balance_weight = 0.3 # Adjust based on machine utilization

        def operation_priority(op):
            """Calculate priority based on adaptive weighting."""
            job_id = op['job_id']
            ...
            # Find the best machine for this operation (SPT, Load balancing).
            ...
            shortest_time = min(times) if times else float('inf')

            load_on_machine = machine_available_time[best_machine] if best_machine is not None else 0

            # Calculate Priority
            priority = (
                makespan_urgency * shortest_time +
                remaining_ops_weight / (job_remaining_operations[job_id] + 1e-6) +  # Avoid division by zero
                load_balance_weight * load_on_machine
            )
            return priority
        ...
        #Re-evaluate best machine, given it may be different than that used in priority function
        for i in range(len(machines)):
            machine = machines[i]
            time = times[i]
            ...
            if op['op_idx'] == job_current_operation[op['job_id']]:
                ...
                for job_schedule in schedule.values():
                    for scheduled_op in job_schedule:
                        if scheduled_op['Operation'] == op['op_idx'] and scheduled_op['job_id'] == op['job_id']:
                            is_scheduled = True
                            break
                    if is_scheduled:
                        break
