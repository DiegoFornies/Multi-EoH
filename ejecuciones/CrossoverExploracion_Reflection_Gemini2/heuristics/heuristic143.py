...
def heuristic(input_data):
    """
    A heuristic for FJSSP balancing SPT and machine load.
    Prioritizes shorter processing times on lightly loaded machines.
    """
    ...
    while available_operations:
        def operation_priority(op):
            ...
            if best_machine is None:
                return float('inf')
            load_factor = 1 + (machine_load[best_machine] / (sum(machine_load.values()) + 1e-9))
