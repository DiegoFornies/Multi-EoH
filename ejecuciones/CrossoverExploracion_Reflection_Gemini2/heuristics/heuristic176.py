...
def heuristic(input_data):
    """Schedules jobs using a shortest remaining processing time (SRPT) dispatching rule."""
    ...
    for job_id, job in jobs_data.items():
        for op_idx, (machines, times) in enumerate(job):
            ...
            if job_id not in remaining_processing_time:
                remaining_processing_time[job_id] = 0
            remaining_processing_time[job_id] += min(times) # Use min time for initial estimation
    ...
    while operations:
        ...
        remaining_processing_time[job_id] -= processing_time # Update
