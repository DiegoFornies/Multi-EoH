...
def heuristic(input_data):
    """
    Combines SPT and machine load while considering job dependencies for FJSSP.
    Uses dynamic weights to prioritize makespan and balance.
    """
    ...
    while available_operations:
        ...
        def operation_priority(op):
            ...
            for mach in available_machines:
              load_times.append(machine_available_time[mach])

            avg_machine_load = sum(load_times) / len(load_times) if load_times else 0
            return min_time + 0.05 * avg_machine_load
