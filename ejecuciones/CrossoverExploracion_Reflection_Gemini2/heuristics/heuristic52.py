...
def heuristic(input_data):
    """Combines earliest completion & ready operation list."""
    ...
    while ready_operations:
        ready_operations.sort(key=lambda x: min(max(machine_available_times[m],
                                                      job_completion_times[x[0]]) for m in
                                                 jobs_data[x[0]][x[1]][0]))
