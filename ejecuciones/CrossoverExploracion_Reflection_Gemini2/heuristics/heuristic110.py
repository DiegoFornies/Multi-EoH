...
def heuristic(input_data):
    """Combines SPT, load balancing, job remaining, and separation time."""
    ...
    while available_operations:
        # SPT, remaining ops, and machine load.
        available_operations.sort(key=lambda op: (job_remaining_operations[op['job_id']], min(op['times']), calculate_machine_load(op['machines'][0]) if len(op['machines']) > 0 else 0))
