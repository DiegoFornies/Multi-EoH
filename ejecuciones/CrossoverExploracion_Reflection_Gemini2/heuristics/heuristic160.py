...
def heuristic(input_data):
    ...
    Prioritizes jobs with fewer remaining operations to improve makespan.
    """
    ...
    job_num_operations = {j: len(jobs_data[j]) for j in jobs_data}
    ...
    def calculate_priority(op):
        """Calculate the priority of an operation."""
        remaining_ops = job_num_operations[op['job_id']] - op['op_idx'] + 1
        min_time = min(op['times'])
        # Balance between SPT and remaining operations
        priority = min_time + (0.1 * remaining_ops)
        return priority
