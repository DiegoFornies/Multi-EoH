...
def heuristic(input_data):
    """Schedules jobs using a shortest remaining processing time (SRPT) approach."""
    ...
    for job in range(1, n_jobs + 1):
        remaining_processing_times[job] = sum(jobs_data[job][i][1][0] for i in range(len(jobs_data[job]))) #Assuming machine 0 has minimal time for the job.
    ...
    while eligible_operations:
        ...
        remaining_processing_times[best_job] -= best_time
