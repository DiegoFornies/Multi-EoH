...
def heuristic(input_data):
    """Schedules jobs by balancing makespan and machine load."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                potential_starts[m] = start_time
                
            # Sort potential start times by machine load and start time
            sorted_machines = sorted(potential_starts.items(), key=lambda item: (machine_available_times[item[0]], item[1]))
            
            # Iterate sorted machines and find best choice
            for m, start_time in sorted_machines:
                m_idx = machines.index(m) # Find correct index
