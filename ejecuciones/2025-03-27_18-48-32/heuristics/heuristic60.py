...
def heuristic(input_data):
    """Schedules jobs dynamically, balancing makespan and machine load."""
    ...
    while eligible_operations:
        ...
        best_time = None
        best_job = None
        load_imbalance = float('inf') # track load imbalance among machines
        
        for job, op_num in eligible_operations:
            machines, times = jobs_data[job][op_num - 1]
            
            # Find best machine and processing time based on makespan and load
            for m_idx, m in enumerate(machines):
                start_time = max(machine_available_times[m], job_completion_times[job])
                current_load_imbalance = max(machine_available_times.values()) - machine_available_times[m]
                if start_time < earliest_start or (start_time == earliest_start and current_load_imbalance < load_imbalance):
