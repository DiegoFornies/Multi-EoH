...
def heuristic(input_data):
    """Schedules jobs balancing makespan, load, and EDD."""
    ...
    job_due_dates = {job: sum(sum(times) / len(times) for _, times in jobs_data[job]) for job in range(1, n_jobs + 1)} # Estimate due date as total avg. proc. time
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # EDD Bonus: Jobs with earlier due dates get scheduling priority
                due_date_bonus = (job_due_dates[job] - job_completion_times[job]) * -0.01 if (job_due_dates[job] - job_completion_times[job]) > 0 else 0  #negative if late, bonus if early.

                adjusted_start_time = start_time + load_penalty + due_date_bonus
