...
def heuristic(input_data):
    """Combines SPT, EDD, and adaptive machine load balancing."""
    ...
    while any(remaining_operations.values()):
        ...
        # Prioritize: EDD, then SPT with load balancing.
        eligible_operations.sort(key=lambda x: (x[4], min([(times[machines.index(m)] + machine_available_time[m],m) for m in machines])[0]))

        job, op_num, machines, times, _ = eligible_operations[0]

        # Choose machine: SPT with Adaptive Load balancing
        best_machine = None
        min_end_time = float('inf')

        for machine in machines:
            processing_time = times[machines.index(machine)]
            start_time = max(machine_available_time[machine], job_completion_time[job] if job in schedule else 0)
        ...
        start_time = max(machine_available_time[best_machine], job_completion_time[job] if job in schedule else 0)
        end_time = start_time + processing_time
