...
def heuristic(input_data):
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            # Evaluate each feasible machine for the operation
            for m_idx, m in enumerate(machines):
                ...
                load_factor = len(machine_assignments[m]) 
                
                # Adjusted end time: prioritize less loaded machines
                adjusted_end_time = end_time + load_factor
                
                if adjusted_end_time < earliest_start:
                    earliest_start = adjusted_end_time
                    best_op = op_num
                    best_machine = m
