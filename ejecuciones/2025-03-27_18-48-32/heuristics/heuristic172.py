...
def heuristic(input_data):
    """Combines load balancing and makespan minimization."""
    ...
    while eligible_operations:
        best_op = None
        earliest_end = float('inf')  # Minimize end time
        best_machine = None
        best_time = None
        best_job = None
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Prioritize less loaded machines, and makespan
                load_factor = machine_load[m] * 0.05
                weighted_end_time = end_time + load_factor

                if weighted_end_time < best_local_end:
                    best_local_end = weighted_end_time
                    best_local_machine = m
                    best_local_time = times[m_idx]

            if best_local_end < earliest_end:
                earliest_end = best_local_end
                best_op = op_num
