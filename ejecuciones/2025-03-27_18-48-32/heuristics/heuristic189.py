...
def heuristic(input_data):
    """Schedules jobs by combining EDD and SPT with urgency."""
    ...
    while any(remaining_operations.values()):
        ...
        for job, operations in remaining_operations.items():
            if operations:
                ...
                processing_time = min(times)  # SPT element
                time_to_due = job_due_time[job] - job_completion_time[job]  # EDD element
                urgency = time_to_due / processing_time if processing_time > 0 else float('inf') # Urgency = EDD/SPT
                eligible_operations.append((job, op_num, machines, times, urgency))

        eligible_operations.sort(key=lambda x: x[4]) # sort by urgency

        job, op_num, machines, times, _ = eligible_operations[0]
        best_machine = min(machines, key=lambda m: machine_available_time[m]) # machine load balancing
