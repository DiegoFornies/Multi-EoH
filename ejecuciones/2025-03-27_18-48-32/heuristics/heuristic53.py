...
def heuristic(input_data):
    """FJSSP heuristic: Hybrid approach balancing SPT and EDD with machine load."""
    ...
    while any(remaining_operations.values()):
        ...
        # Hybrid: SPT within EDD groups, balancing machine load.
        eligible_operations.sort(key=lambda x: (x[4], min(x[3]), machine_available_time[min(x[2], key=lambda m: machine_available_time[m])]))
