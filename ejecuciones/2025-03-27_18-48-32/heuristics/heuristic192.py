...
def heuristic(input_data):
    """Combines shortest processing time with load balancing and EDD."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Adaptive priority: consider machine load, job urgency, SPT
                load_penalty = machine_load[m] * 0.01  # Reduce impact of load
                urgency_bonus = job_completion_times[job] * 0.02 # EDD
                spt_bonus = times[m_idx] * 0.01 # SPT

                adjusted_start_time = start_time + load_penalty - urgency_bonus + spt_bonus

                if adjusted_start_time < best_start:
                    best_start = adjusted_start_time
                    best_op = op_num
                    best_machine = m
