...
def heuristic(input_data):
    """Schedules jobs balancing makespan, load, and job urgency."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Dynamic priority: consider machine load and job urgency
                load_penalty = machine_load[m] * 0.01 # Reduced penalty
                urgency_bonus = (job_completion_times[job] / (sum(machine_load.values()) + 1e-6)) * 0.03 if sum(machine_load.values()) > 0 else 0 # Reduced bonus
