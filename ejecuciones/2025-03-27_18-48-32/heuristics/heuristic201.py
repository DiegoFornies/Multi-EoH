...
def heuristic(input_data):
    """Combines EDD, SPT, and adaptive machine load balancing for FJSSP."""
    ...
    for job in range(1, n_jobs + 1):
        job_due_time[job] = sum(min(times) for machines, times in jobs_data[job]) * 1.5 # Estimate due time
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Adaptive load penalty (adjust dynamically)
                load_penalty = machine_load[m] * 0.1  # Initial penalty
                #Reduce penalty if job is urgent (close to completion)
                remaining_ops = len(jobs_data[job]) - op_num
                urgency_factor = max(0, 1 - remaining_ops / len(jobs_data[job])) if len(jobs_data[job]) > 0 else 0
                load_penalty *= (1 - urgency_factor) #Reduce penalty if job near completion

                #EDD based earliness factor
                earliness = max(0, job_due_time[job] - (start_time + processing_time)) / job_due_time[job]

                adjusted_start_time = start_time + load_penalty - earliness

                if adjusted_start_time < best_local_start:
                    best_local_start = adjusted_start_time
                    best_local_machine = m
                    best_local_time = processing_time
