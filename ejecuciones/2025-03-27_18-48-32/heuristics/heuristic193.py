...
def heuristic(input_data):
    """FJSSP heuristic: SPT + EDD + load balancing."""
    n_jobs = input_data['n_jobs']
    n_machines = input_data['n_machines']
    jobs_data = input_data['jobs']

    machine_available_times = {m: 0 for m in range(n_machines)}
    job_due_dates = {job: sum(times[0][0] for machines, times in jobs_data[job]) for job in jobs_data}
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                start_time = max(machine_available_times[m], scheduled_operations[job][-1]['End Time'] if scheduled_operations[job] else 0)

                # SPT + EDD + Load balancing
                spt_penalty = times[m_idx] * 0.1
                edd_bonus = (job_due_dates[job] - start_time) * 0.05
                load_penalty = machine_load[m] * 0.02

                adjusted_start_time = start_time + spt_penalty - edd_bonus + load_penalty
        ...
        start_time = max(machine_available_times[best_machine], scheduled_operations[best_job][-1]['End Time'] if scheduled_operations[best_job] else 0)
