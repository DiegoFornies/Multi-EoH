...
def heuristic(input_data):
    """FJSSP heuristic: SPT with dynamic machine selection."""
    ...
    while any(remaining_operations.values()):
        ...
        # Dynamically select machine based on a combined score of availability and load
        machine_scores = {}
        for idx, machine in enumerate(machines):
            machine_scores[machine] = machine_available_time[machine] #+ machine_load.get(machine, 0) # Include machine load if tracking it
        best_machine = min(machine_scores, key=machine_scores.get)
