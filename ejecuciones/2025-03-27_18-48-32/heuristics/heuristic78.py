...
def heuristic(input_data):
    """Schedules jobs balancing makespan & job completion."""
    ...
    while any(remaining_operations.values()):
        ...
        for job, operations in remaining_operations.items():
            if operations:
                ...
                eligible_operations.append((job, op_num, job, machines, times))

        # Prioritize jobs that have more remaining operations.
        eligible_operations.sort(key=lambda x: (-len(remaining_operations[x[2]])))

        job, op_num, job_id, machines, times = eligible_operations[0]
