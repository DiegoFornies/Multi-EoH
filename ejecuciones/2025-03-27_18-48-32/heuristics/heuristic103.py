...
def heuristic(input_data):
    """Schedules jobs balancing makespan and machine load with dynamic priority."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Dynamic priority: consider machine load and job urgency (completion time)
                load_penalty = machine_load[m] * 0.05 # Adjust load penalty
                urgency_bonus = (job_completion_times[job] / sum(machine_load.values())) if sum(machine_load.values()) > 0 else 0 # Adjust urgency
