...
def heuristic(input_data):
    """Schedules jobs by considering processing time, job urgency, and machine load."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            #Urgency is determined by the operations left and total completion time
            remaining_operations = len(jobs_data[job]) - op_num
            job_urgency = remaining_operations + job_completion_times[job]
            
            for m_idx, m in enumerate(machines):
                start_time = max(machine_available_times[m], job_completion_times[job])
                processing_time = times[m_idx]
                
                # Adaptive priority: considers processing time, urgency, and machine load
                priority = processing_time + 0.5 * job_urgency + 0.1 * machine_available_times[m]
