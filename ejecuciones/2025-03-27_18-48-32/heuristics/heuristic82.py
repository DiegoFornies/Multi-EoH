...
def heuristic(input_data):
    ...
    while any(remaining_operations.values()):
        ...
        # Prioritize EDD, then balance machine load
        eligible_operations.sort(key=lambda x: (x[4], machine_load[min(x[2], key=lambda m: machine_load[m])]))

        job, op_num, machines, times, _ = eligible_operations[0]

        # Assign to the least loaded machine
        best_machine = min(machines, key=lambda m: machine_load[m])
