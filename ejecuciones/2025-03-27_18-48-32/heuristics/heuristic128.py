...
def heuristic(input_data):
    """Schedules jobs using a shortest remaining processing time (SRPT) policy."""
    ...
    for job, operations in jobs_data.items():
        remaining_times[job] = sum(min(times) for _, times in operations)
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                if start_time < earliest_start and remaining_times[job] < shortest_remaining:
                    earliest_start = start_time
                    best_op = op_num
                    best_machine = m
                    best_time = times[m_idx]
                    best_job = job
                    shortest_remaining = remaining_times[job]
        ...
        remaining_times[best_job] -= best_time # this is incorrect, it should be adjusted based on the machine assigned
