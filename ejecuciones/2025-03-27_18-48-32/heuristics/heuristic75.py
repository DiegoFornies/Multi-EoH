...
def heuristic(input_data):
    ...
    for job in range(1, n_jobs + 1):
        job_total_work[job] = sum(min(times) for machines, times in jobs_data[job])
    ...
    while any(remaining_operations.values()):
        ...
        for job, operations in remaining_operations.items():
            if operations:
                ...
                min_time = min(times)
                # Calculate slack: (total work - completed work) / remaining ops
                slack = (job_total_work[job] - (job_completion_time[job] - (0 if len(schedule[job]) == 0 else sum([op['Processing Time'] for op in schedule[job]])))) / len(operations)
                eligible_operations.append((job, op_num, machines, times, slack))
