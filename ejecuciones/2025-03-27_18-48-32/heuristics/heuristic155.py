...
def heuristic(input_data):
    ...
    while any(remaining_operations.values()):
        ...
        # Prioritize Earliest Due Date, then least loaded machine, then fewest jobs
        eligible_operations.sort(key=lambda x: (x[4], machine_available_time[min(x[2], key=lambda m: (machine_available_time[m], len(machine_assignments[m])))]))
        ...
        best_machine = min(machines, key=lambda m: (machine_available_time[m], len(machine_assignments[m])))
