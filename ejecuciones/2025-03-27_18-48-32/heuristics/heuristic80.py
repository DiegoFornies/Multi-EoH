...
def heuristic(input_data):
    """Schedules jobs using a dynamic shortest processing time heuristic."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Dynamic prioritization: Favor operations that help balance machine load.
                # Calculate machine utilization for the current machine
                machine_utilization = 0
                if machine_available_times[m] > 0:
                    machine_utilization = machine_available_times[m] / sum(machine_available_times.values()) if sum(machine_available_times.values()) else 0

                # Adjust start time based on machine utilization
                # Give preference to underutilized machine
                adjusted_start_time = start_time + (machine_utilization) #Lower the start time when machine load is small
