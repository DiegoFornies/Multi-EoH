...
def heuristic(input_data):
    """Schedules jobs considering processing time, machine load, and job completion."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                # Consider machine load in start time calculation
                start_time = max(machine_available_times[m], job_completion_times[job])
                
                # Prioritize machines with lighter loads
                load_factor = machine_load[m] # Machine load (lower is better)
                adjusted_start_time = start_time + load_factor # Add Load factor to start time
        ...
        machine_load[best_machine] += best_time #Increment machine load
