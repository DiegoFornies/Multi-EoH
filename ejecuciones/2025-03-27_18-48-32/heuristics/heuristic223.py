...
def heuristic(input_data):
    """Combines EDD and SPT with machine load for FJSSP."""
    ...
    job_due_time = {job: sum(min(times) for machines, times in jobs_data[job]) * 1.5 for job in range(1, n_jobs + 1)}
    ...
    while any(remaining_operations.values()):
        ...
        for job, operations in remaining_operations.items():
            if operations:
                ...
                min_time = min(times)
                eligible_operations.append((job, op_num, machines, times, job_due_time[job], min_time))

        # Prioritize: EDD, SPT, then machine load
        eligible_operations.sort(key=lambda x: (x[4], x[5], machine_available_time[min(x[2], key=lambda m:machine_available_time[m])]))
