...
def heuristic(input_data):
    """FJSSP heuristic: Balances load, SPT, and EDD using dynamic weights."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Dynamic priority: consider machine load, job urgency, and SPT
                load_penalty = machine_load[m] * 0.02  # Reduced load penalty
                urgency_bonus = (job_completion_times[job] / sum(machine_load.values())) if sum(machine_load.values()) > 0 else 0
                spt_bonus = times[m_idx] * 0.01  # SPT bonus (smaller processing time is better)

                adjusted_start_time = start_time + load_penalty - urgency_bonus + spt_bonus
