...
def heuristic(input_data):
    """Schedules jobs by considering job slack and machine load."""
    ...
    while eligible_operations:
        ...
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                # Calculate job slack: estimated remaining processing time until completion
                remaining_time = 0
                for future_op_num in range(op_num + 1, len(jobs_data[job]) + 1):
                    future_machines, future_times = jobs_data[job][future_op_num - 1]
                    remaining_time += min(future_times)  # optimistic estimate
                
                slack = machine_available_times[m] - end_time + remaining_time
                if slack < min_slack:
                    min_slack = slack
                    best_op = op_num
                    best_machine = m
