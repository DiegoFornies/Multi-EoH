...
def heuristic(input_data):
    ...
    while eligible_operations:
        ...
        # Iterate through eligible operations to find earliest start time
        for job, op_num in eligible_operations:
            ...
            for m_idx, m in enumerate(machines):
                ...
                #Load balance factor
                load_factor = machine_available_times[m] / sum(machine_available_times.values()) if sum(machine_available_times.values()) > 0 else 0
                #Adjust start time based on load balance
                adjusted_start_time = start_time + (load_factor * 0.1 * start_time)
