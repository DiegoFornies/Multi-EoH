...
class Reader:
    ...
    def get_crossoverI_reflection_prompt(self, long_reflection, parent1, parent2):
        role_init = self.get_role()
    ...
    def get_crossoverII_reflection_prompt(self, long_reflection, parent1, parent2):
        role_init = self.get_role()
        system_prompt = self.read_file(f'{self.prompt_path}/system_generator_prompt.txt').replace('{role_init}', role_init)
    ...
    def get_mutation_reflection_prompt(self, long_reflection, parent):
        role_init = self.get_role()
        system_prompt = self.read_file(f'{self.prompt_path}/system_generator_prompt.txt').replace('{role_init}', role_init)
    ...
    def get_crossoverI_noreflection_prompt(self, parent1, parent2):
        role_init = self.get_role()
        system_prompt = self.read_file(f'{self.prompt_path}/system_generator_prompt.txt').replace('{role_init}', role_init)
        user_prompt = self.read_file(f'{self.prompt_path}/noreflection/user_crossover_prompt.txt')
        task_description = self.get_task_description()
    ...
    def get_crossoverII_noreflection_prompt(self, parent1, parent2):
        role_init = self.get_role()
        system_prompt = self.read_file(f'{self.prompt_path}/system_generator_prompt.txt').replace('{role_init}', role_init)
        user_prompt = self.read_file(f'{self.prompt_path}/noreflection/user_crossoverII_prompt.txt')
        task_description = self.get_task_description()

        user_prompt = user_prompt.replace('{task_description}', task_description).replace('{parent1}', parent1.code).replace('{parent2}', parent2.code)
        return system_prompt, user_prompt
    
    def get_mutation_noreflection_prompt(self, parent):
        role_init = self.get_role()
        system_prompt = self.read_file(f'{self.prompt_path}/system_generator_prompt.txt').replace('{role_init}', role_init)
        user_prompt = self.read_file(f'{self.prompt_path}/noreflection/user_mutation_prompt.txt')
        task_description = self.get_task_description()

        user_prompt = user_prompt.replace('{task_description}', task_description).replace('{parent}', parent.code)
        return system_prompt, user_prompt